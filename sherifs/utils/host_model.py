...
def build(XMLfile, host_model_file, Lon_bg, Lat_bg):
    ...
    for line in lines_of_the_host_file:
        ...
        if simple_fault == True or complex_fault == True:
            ...
            if '</gml:posList>' in line:
                ...
                lon_lat = ''
                for line_lon_lat in lines_of_the_host_file[
                        line_start_lonlat:line_stop_lonlat + 1]:
            ...
            if print_source == True or subduction_source == True:
                if source_read == True:
                    line_to_print = lines_of_the_host_file[
                        line_start][:index_id] + '1111' + lines_of_the_host_file[
                            line_start][index_id:]
                    XMLfile.write(line_to_print + '\n')
                    for line_to_print in lines_of_the_host_file[line_start +
                                                                1:line_end + 1]:
                        XMLfile.write(line_to_print + '\n')
    ...
    #                        Xsing_points_bg =[]
                    ...
                    for index_pt_z in range(len(listpoint_in_bg)):
                        if index_pt_z != 0:
                            if listpoint_in_bg[index_pt_z] != listpoint_in_bg[
                                    index_pt_z - 1]:
                                nb_Xing_zone += 1
                        else:
                            if listpoint_in_bg[-1] != listpoint_in_bg[
                                    index_pt_z]:
                                nb_Xing_zone += 1
                    ...
                    for index_pt_bg in range(len(bg_point_inzone)):
                        if index_pt_bg != 0:
                            if bg_point_inzone[index_pt_bg] != bg_point_inzone[
                                    index_pt_bg - 1]:
                                nb_Xing_bg += 1
                        else:
                            if bg_point_inzone[-1] != bg_point_inzone[
                                    index_pt_bg]:
                                nb_Xing_bg += 1
                    number_of_Xing = max([nb_Xing_zone, nb_Xing_bg])

                    if sum(bg_point_inzone) == len(bg_point_inzone) and sum(
                            listpoint_in_bg
                    ) == 0:  # if the background if completely included in the zone
                        lon_zone_modif = []
                        lat_zone_modif = []
                        for point in points_zone:
                            lon_zone = point[0]
                        ...
                        for lon_bg, lat_bg in zip(Lon_bg, Lat_bg):
                            distances.append(
                                distance(lon_bg, lat_bg, points_zone[-1][0],
                                         points_zone[-1][1]))
                        index_dist_min = np.argmin(distances)
                        lon_bg_modif = Lon_bg[
                            index_dist_min:] + Lon_bg[:index_dist_min]
                        lat_bg_modif = Lat_bg[
                            index_dist_min:] + Lat_bg[:index_dist_min]
                        if (distance(lon_bg_modif[-1], lat_bg_modif[-1],
                                     points_zone[0][0], points_zone[0][1]) >
                                distance(lon_bg_modif[0], lat_bg_modif[0],
                                         points_zone[0][0], points_zone[0][1])):
                        ...
                        line1 = [[points_zone[-1][0], points_zone[-1][1]],
                                 [lon_bg_modif[0], lat_bg_modif[0]]]
                        line2 = [
                            [
                                np.mean([lon_bg_modif[0], points_zone[-1][0]]) +
                                0.0001,
                                np.mean([lat_bg_modif[0], points_zone[-1][1]])
                            ],
                            [
                                np.mean([lon_bg_modif[0], points_zone[-1][0]]),
                                np.mean([lat_bg_modif[-1], points_zone[-1][1]])
                            ]
                        ]
                        ...
                        if x != 'no_intesection':
                            if (points_aligned([
                                    np.mean(
                                        [lon_bg_modif[0], points_zone[-1][0]]) +
                                    0.0001,
                                    np.mean(
                                        [lat_bg_modif[0], points_zone[-1][1]])
                            ], [
                                    np.mean(
                                        [lon_bg_modif[0], points_zone[-1][0]]),
                                    np.mean(
                                        [lat_bg_modif[-1], points_zone[-1][1]])
                            ], [x, y]) == False):
                        ...
                        for point in points_zone:
                            ...
                            lat_zone = point[1]
                            if listpoint_in_bg[
                                    index_point_z] == 0:  #if the point is not in the bg, we add it
                                lon_zone_modif.append(lon_zone)
                                lat_zone_modif.append(lat_zone)

                            index_bg_intercept = None
                            if index_point_z != len(
                                    points_zone) - 1:  #it's not the last point
                                if (listpoint_in_bg[index_point_z] == 0 and
                                        listpoint_in_bg[index_point_z + 1] == 1
                                   ) or (listpoint_in_bg[index_point_z] == 1 and
                                         listpoint_in_bg[index_point_z + 1]
                                         == 0):  #it's a crossing point

                                    index_point_bg = 0
                                    for lon_bg, lat_bg in zip(Lon_bg, Lat_bg):
                                        if index_point_bg != len(
                                                bg_point_inzone
                                        ) - 1:  #it's not the last point
                                            #find the intersecting point
                                            line1 = [
                                                [lon_zone, lat_zone],
                                                [
                                                    points_zone[index_point_z +
                                                                1][0],
                                                    points_zone[index_point_z +
                                                                1][1]
                                                ]
                                            ...
                                            line2 = [
                                                [lon_bg, lat_bg],
                                                [
                                                    Lon_bg[index_point_bg + 1],
                                                    Lat_bg[index_point_bg + 1]
                                                ]
                                            ]
                                            ...
                                            if x != 'no_intesection':
                                                if (points_aligned(
                                                    [lon_bg, lat_bg], [
                                                        Lon_bg[index_point_bg +
                                                               1],
                                                        Lat_bg[index_point_bg +
                                                               1]
                                                    ], [x, y]) == True
                                                   ) and (points_aligned([
                                                       lon_zone, lat_zone
                                                    ...
                                                    #print('Xing1')
                                                    lon_zone_modif.append(
                                                        float(x))
                                                    lat_zone_modif.append(
                                                        float(y))
                                                    if bg_point_inzone[
                                                            index_point_bg] == 1:
                                ...
                                if (listpoint_in_bg[index_point_z] == 0 and
                                        listpoint_in_bg[0]
                                        == 1) or (listpoint_in_bg[index_point_z]
                                                  == 1 and listpoint_in_bg[0]
                                                  == 0):  #it's a crossing point
                                    index_point_bg = 0
                                    for lon_bg, lat_bg in zip(Lon_bg, Lat_bg):
                                        if index_point_bg != len(
                            ...
                            if listpoint_in_bg[
                                    index_point_z] == 0 and index_bg_intercept != None:  #only need to write bg points if the zone point is outside of the bg
                                lon_bg_modif = Lon_bg[
                                    index_bg_intercept:] + Lon_bg[:
                                                                  index_bg_intercept]
                                lat_bg_modif = Lat_bg[
                                    index_bg_intercept:] + Lat_bg[:
                                                                  index_bg_intercept]
                                bg_point_inzone_modif = bg_point_inzone[
                                    index_bg_intercept:] + bg_point_inzone[:
                                                                           index_bg_intercept]
                                if index_bg_intercept != 0 and bg_point_inzone_modif[
                                        -1] == 1:
                                    lon_bg_modif = list(reversed(lon_bg_modif))
                                    lat_bg_modif = list(reversed(lat_bg_modif))
                                    bg_point_inzone_modif = list(
                                        reversed(bg_point_inzone_modif))
                    ...
                    area_of_the_zone_modified = area_of_polygon(
                        x, y)  #to be verified!!
                    ...
                    if number_of_Xing >= 3:
                        ...
                        indexes_for_test_init = [
                            list(j) for i, j in groupby(listpoint_in_bg)
                        ]
                        indexes_for_test = []
                        index = 0
                        ...
                        for indexes in list(indexes_for_test):
                            ...
                            if len(indexes) > 1:
                                for index in indexes:
                                    i = 0
                                    for lon, lat in zip(lon_zone_modif,
                                                        lat_zone_modif):
                                        if lon == ColX[index] and lat == ColY[
                                                index]:
                                ...
                                for lon, lat in zip(lon_zone_modif,
                                                    lat_zone_modif):
                                    if lon == ColX[indexes[0]] and lat == ColY[
                                            indexes[0]]:
                                        indexes_modif.append(i)
                                    i += 1
                        ...
                        for indexes in list(indexes_for_test_modif):
                            if len(indexes) > 1:
                                ...
                                area_of_the_zone_to_test = area_of_polygon(
                                    x, y)  #to be verified!!
                ...
                if sum(bg_point_inzone) == len(bg_point_inzone) and sum(
                    ...
                    ratio_areas = (area_of_the_zone -
                                   area_of_the_bg) / area_of_the_zone
                ...
                if type_mfd == True and source_read == True:
                    ...
                    if line.find('aValue="') == -1:
                        print(
                            'PROBLEM with reading the host file to find the a value'
                        )
                    a_str = line[index_a:]
                    i = a_str.find('"')
                    a = float(a_str[:i])
                    a_modif = a * ratio_areas
                    line_mfd_modif = line[:index_a] + str(
                        a_modif) + line[line.find('"', index_a + 1):]
                ...
                if type_increment == True and source_read == True:
                    mfd_str = ''
                    for line_mfd_str in lines_of_the_host_file[
                            line_mfd_start_number:line_mfd_stop_number + 1]:
                ...
                if source_read == True:
                    line_to_print = lines_of_the_host_file[
                        line_start][:index_id] + '1111' + lines_of_the_host_file[
                            line_start][index_id:]
                    XMLfile.write(line_to_print + '\n')
                    for line_to_print in lines_of_the_host_file[
                            line_start + 1:line_start_lonlat]:
                        XMLfile.write(line_to_print + '\n')
                    ...
                    if number_of_Xing >= 3:
                        print(
                            'POSSIBLE ERROR : please check if the host model is incorporate correctly, problems might have occured!!!'
                        )
                        print(lines_of_the_host_file[line_start][-9:-2],
                              'number_of_Xing=', number_of_Xing)
                        print('ratio_areas', ratio_areas)
                        import matplotlib.pyplot as plt
                        plt.scatter(ColX, ColY, c='b', alpha=0.2)
                        plt.scatter(Lon_bg, Lat_bg, c='r', alpha=0.2)
                        plt.scatter(lon_zone_modif,
                                    lat_zone_modif,
                                    c='k',
                                    alpha=0.2,
                                    marker='s')
                    ...
                    for line_to_print in lines_of_the_host_file[
                            line_stop_lonlat + 1:line_mfd_start_number]:
                        XMLfile.write(line_to_print + '\n')
                    XMLfile.write(line_mfd_modif + '\n')
                    for line_to_print in lines_of_the_host_file[
                            line_mfd_stop_number + 1:line_end + 1]:
                        XMLfile.write(line_to_print + '\n')
        line_number += 1
