...
class Geom_scenar:
    ...
    def initialize(self):
        ...
        for fault in faults_names:
            ...
            lon_fault = np.array(
                list(map(lambda i: self.Longitudes[i], index[0])))
            lon_fault.resize(faults_lon_ini[index_fault][:].shape)
            lat_fault = np.array(
                list(map(lambda i: self.Latitudes[i], index[0])))
            lat_fault.resize(faults_lat_ini[index_fault][:].shape)
            faults_lon_ini[
                index_fault][:] = faults_lon_ini[index_fault][:] + lon_fault
            faults_lat_ini[
                index_fault][:] = faults_lat_ini[index_fault][:] + lat_fault
        ...
        tmp_f_names = []
        for fault in faults_names:
            index_zeros = np.where((faults_lat_ini[index_fault] == 0.) &
                                   (faults_lon_ini[index_fault] == 0.))
            ...
            if depths_fault[0] == 'sf':
                for i in range(len(faults_lon[index_fault]) - 1):
                    dist += self.distance(faults_lon[index_fault][i],
                                          faults_lat[index_fault][i],
                                          faults_lon[index_fault][i + 1],
                                          faults_lat[index_fault][i + 1])
            else:
                for i in range(len(faults_lon[index_fault]) - 1):
    ...
    def FaultGeometry(self):
        if not ".geojson" in self.File_geom:
            ...
            index_model = np.where(
                np.array(Column_model_name) == self.Model_name)
