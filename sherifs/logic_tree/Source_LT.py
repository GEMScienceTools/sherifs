...
class S_LT():
    ...
    def initialize(self):
        ...
        for self.model_i in self.selected_Model:
            ...
            self.nb_sc_hyp()
            self.nb_bg_hyp()
            for ScL_i, use_all_i, dim_i in zip(self.selected_ScL,
                                               self.use_all_ScL_data,
                                               self.dimention_used):
                index_mfd = 0
                for mfd_i in self.mfd_hyp:
                    for bmin_i, bmax_i in zip(self.bmin_hyp[index_mfd],
                                              self.bmax_hyp[index_mfd]):
                        for bg_hyp_i in self.bg_names:
                            for sc_name in self.sc_names:
                                branch_i = [
                                    self.model_i, ScL_i, use_all_i, dim_i,
                                    mfd_i,
                                    str(bmin_i) + '_' + str(bmax_i), bg_hyp_i,
                                    sc_name
                                ]
    ...
    def windows_ScL(self):
        ...
        for i in range(self.nb_of_scl):
            ...
            self.ScL_name["ScLname{0}".format(i)] = Combobox(
                self.w_ScL,
                textvariable=self.ScLname_used["ScLname{0}".format(i)],
                values=self.available_scaling_laws,
                state='readonly',
                height=5,
                width=30)
            ...
            self.use_all_ScL_data_button = Checkbutton(
                self.w_ScL,
                text="Respect fault kinematic",
                variable=self.var["use_all_ScL_data_{0}".format(i)],
                onvalue="False",
                offvalue="True")
            self.use_all_ScL_data_button.grid(column=4, row=row_i)

            self.ScLSelect_dim_used["dimention_used_{0}".format(
                i)] = StringVar()
            self.dimention_used_box["dimention_used_{0}".format(i)] = Combobox(
                self.w_ScL,
                textvariable=self.ScLSelect_dim_used[
                    "dimention_used_{0}".format(i)],
                values=['Area', 'Length'],
                state='readonly',
                height=5,
                width=30)
            self.dimention_used_box["dimention_used_{0}".format(i)].current(0)
            self.dimention_used_box["dimention_used_{0}".format(i)].grid(
                column=8, row=row_i)
            row_i += 1
    ...
    def OK_Scl(self):
        ...
        self.selected_ScL = []
        for i in range(self.nb_of_scl):
            self.selected_ScL.append(
                self.ScL_name["ScLname{0}".format(i)].get())
            if self.var["use_all_ScL_data_{0}".format(i)].get() == 'False':
                self.use_all_ScL_data.append(False)
            else:
                self.use_all_ScL_data.append(True)
            self.dimention_used.append(
                self.dimention_used_box["dimention_used_{0}".format(i)].get())

        self.w_ScL.destroy()

        check_ScL = []
        for selected_ScL_i, use_all_ScL_data, dimention_used in zip(
                self.selected_ScL, self.use_all_ScL_data, self.dimention_used):
            line = str(selected_ScL_i) + str(use_all_ScL_data) + str(
                dimention_used)
            if not line in check_ScL:
                ...
                messagebox.showerror(
                    'Error',
                    'One scaling law has been selected twice\n please start again'
                )
        self.win_model()
    ...
    def win_model(self):
        ...
        label.pack()
        self.Box_Model = Combobox(self.w_model,
        ...
        self.Box_Model.pack()
        bou_add_Model = Button(self.w_model,
                               text=u'Add Model',
                               command=self.Add_Model)
        bou_add_Model.pack()
        self.list_Model = Listbox(self.w_model, height=5, width=30)
        self.list_Model.pack()
        bou_del_Model = Button(self.w_model,
                               text=u'Delete Selected Model',
                               command=self.del_Model)
        bou_del_Model.pack()
        label = Label(self.w_model, text="\n\n\nWhen ready click \"OK\"")
        label.pack()
    ...
    def nb_mfd_hyp(self):
        self.w_mfd_nb = tk.Tk()
        self.w_mfd_nb.title('Number of MFD hypothesis for model : ' +
                            str(self.model_i))
        label = Label(self.w_mfd_nb,
                      text='\nHow many MFD hypothesis for model : ' +
                      str(self.model_i) + ' do you want to use?')
        label.pack()
    ...
    def nb_b_hyp(self):
        self.w_b_nb = tk.Tk()
        self.w_b_nb.title('Number of b value distribution for model : ' +
                          str(self.model_i) + '\nand MFD : ' +
                          str(self.mfd_hyp[self.i]))
        label = Label(
            self.w_b_nb,
            text='\nHow many b value distribution hypothesis for model : ' +
    ...
    def b_hyp(self):
        ...
        row_i = 0
        label = Label(self.w_b,
                      text='\nHypothesis on b value for model ' +
                      str(self.model_i) + ' and MFD : ' +
                      str(self.mfd_hyp[self.i]))
        label.grid(column=0, row=row_i)
        row_i += 1
        for i in range(self.nb_of_b):
            label = Label(self.w_b,
                          text="Hypothesis " + str(i + 1) +
                          " for b min and b max")
            label.grid(column=0, row=row_i)
    ...
    def nb_sc_hyp(self):
        self.w_sc_nb = tk.Tk()
        self.w_sc_nb.title('Number of scenario sets for model : ' +
                           str(self.model_i))
        label = Label(self.w_sc_nb,
                      text='\nHow many scenario sets for model : ' +
                      str(self.model_i) + ' do you want to use?')
        label.pack()
    ...
    def nb_bg_hyp(self):
        self.w_bg_nb = tk.Tk()
        self.w_bg_nb.title('Number of background hypothesis for model ' +
                           str(self.model_i))
        label = Label(self.w_bg_nb,
                      text='\nHow many background hypothesis for model ' +
                      str(self.model_i) + ' do you want to use?')
        label.pack()
    ...
    def bg_hyp(self):
        ...
        self.w_bg = tk.Tk()
        self.w_bg.grid()
        self.w_bg.title('Name of the background hypotheses for ' +
                        str(self.model_i))
        row_i = 0
        label = Label(self.w_bg,
