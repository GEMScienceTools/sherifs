    ...
    def __init__(self, param, calculation_log_file):
        ...
        if param["main"]["background"]["option_bg"] == "smooth":
            ...
            if os.path.isdir(fbgpath):
                list_fbg = [
                    f for f in listdir(fbgpath) if isfile(join(fbgpath, f))
                ]
            else:
        ...
        if not os.path.exists(LT_log_name):
            ...
            if force_rerun in ["False", "false"]:
                if os.path.isfile(path + self.Run_Name + "/LOG/lt_branchs.pkl"):
                    old_branches = pickle.load(
                        open(path + self.Run_Name + "/LOG/lt_branchs.pkl",
                             'rb'))
                    old_indexes = pickle.load(
                        open(path + self.Run_Name + "/LOG/lt_b_id.pkl", 'rb'))
                else:
                    old_branches = []
                    old_indexes = []
            else:
            ...
            try:
                ...
                print(
                    'Error related to the rupture scenario set file \n' +
        ...
        for id in used_id:
            ...
            if branch["run_branch"] == True:
                ...
                if not os.path.exists(b_path):
                    ...
                    if self.param['main']['parameters']['force_rerun'] in [
                            'true', 'True'
                    ]:
                        ...
                        for index_scenario in range(len(rupture_set)):
                            ...
                            if len(faults_in_scenario) > 1:
                                ...
                                for i in range(len(faults_in_scenario)):
                                    if not str(faults_in_scenario[i]).replace(
                                            '\r', '') in faults_done:
                                        scenario["f_%s" % str(i + 1)] = [
                                            str(faults_in_scenario[i]).replace(
                                                '\r', '').replace('\t',
                                                                  '').replace(
                                                                      '\n', '')
                                        ]
                ...
                if last_model != model_hyp:
                    ...
                    if self.param["main"]["fault_input_type"] == "txtsherifs":
                        ...
                        Column_model_name = list(
                            map(lambda i: Prop[i][0], range(len(Prop))))
                        Column_fault_name = list(
                            map(lambda i: Prop[i][1], range(len(Prop))))
                        index_model = np.where(
                            np.array(Column_model_name) == model_hyp)[0]
                        Prop = np.take(Prop, index_model)
                        faults_names = np.array(
                            Column_fault_name[index_model[0]:index_model[-1] +
                                              1])
                    ...
                    geom_scenar = Geometry_scenario.Geom_scenar(
                        faults_names, self.File_geom, model_hyp)
                    ...
                    for i_fault in range(len(faults)):
                        pairs = []
                        for i, j in zip(faults_lon[i_fault],
                                        faults_lat[i_fault]):
                            pair = [i, j]
                    ...
                    if simplify_faults in ["True", "true"]:
                        ...
                        for i_fault in range(len(faults)):
                            faults_lon[i_fault] = [
                                faults_lon[i_fault][0], faults_lon[i_fault][-1]
                            ]
                            faults_lat[i_fault] = [
                                faults_lat[i_fault][0], faults_lat[i_fault][-1]
                            ]

                    self.FaultGeometry(
                        model_hyp
                    )  #extract the geometries from the geometry file

                    print("\t - importing faults properties")
                    re_use = True
                    f_prop_tmp = path + str(
                        self.Run_Name) + '/LOG/' + model_hyp + '_prop.pkl'
                    ...
                    if re_use == False:
                        for Fault_name in faults_names:
                            # extract depth
                            i_d = np.where(
                                np.array(self.Column_Fault_name) == Fault_name)
                            ...
                            width = (lower_sismo_depth -
                                     upper_sismo_depth) / math.sin(
                                         math.radians(dip))
                ...
                Source_model = Source_Model_Creator(
                    b_path, log_path, self.param, model_hyp, rupture_set, smp,
                    self.Domain_in_model, scl[0], scl[1], use_all_ScL_data,
                    mfd_param, mfd_hyp[0], bg_ratio, self.calculation_log_file,
                    faults_names, scenarios_names, faults_data, faults_lon,
                    faults_lat, self.list_fbg, self.fbgpath, branch)
                ...
                b_path = path + self.Run_Name + "/ssm/b_" + str(id)
                list_src_files = [
                    f for f in listdir(b_path) if isfile(join(b_path, f))
                ]

            if not "/ssm/b_" + str(
                    id) + "/single_sec_rup.xml" in list_src_files:
                list_src_files.append("ssm/b_" + str(id) +
                                      "/single_sec_rup.xml")

            # write branch in the logic tree file
            line += ('\t\t\t\t\t<uncertaintyModel> \n')
            if self.param["main"]["parameters"]["use_multiF"] in [
                    'True', 'true'
            ]:
            ...
            line += '\t\t\t\t\t<uncertaintyWeight>' + str(
                round(1. / float(len(branches)), 5)) + '</uncertaintyWeight>\n'
            ...
            for fi in range(len(faults)):
                if str(faults[fi]['properties']['si']) == Name_of_fault:
                    if faults[fi]['properties']['model'] == Model:
                        ...
                        self.upper_sismo_depth = faults[fi]['properties'][
                            'up_s_d']
                        self.lower_sismo_depth = faults[fi]['properties'][
                            'lo_s_d']
                        ...
                        self.shear_mod = faults[fi]['properties'][
                            'shear_modulus']
    ...
    def FaultGeometry(self, Model):
        if not ".geojson" in self.File_geom:
            ...
            self.Longitudes = list(
                map(lambda i: InfosZonage[i][2], index_model[0]))
            self.Latitudes = list(
                map(lambda i: InfosZonage[i][3], index_model[0]))
            self.Depths = list(map(lambda i: InfosZonage[i][4], index_model[0]))
            ...
            Column_Fault_name = []
            for cle in DicoZone.keys():
                indices_ZonesSelec = np.where(
                    np.array(self.Column_Fault_name) == cle)
                ColonneNomZone_inter = np.take(self.Column_Fault_name,
                                               indices_ZonesSelec)
                Longitudes_inter = np.take(self.Longitudes, indices_ZonesSelec)
                Latitudes_inter = np.take(self.Latitudes, indices_ZonesSelec)
                depth_inter = np.take(self.Depths, indices_ZonesSelec)
                ...
                ColonneNomZone_inter = ColonneNomZone_inter[0].tolist()
                compt = 0
                for xx, yy, nn, dd in zip(Longitudes_inter, Latitudes_inter,
                                          ColonneNomZone_inter, depth_inter):
                    compt += 1
                    Longitudes.append(xx)
                    Latitudes.append(yy)
                    Depths.append(dd)
            ...
            for fi in range(len(faults)):
                if faults[fi]['properties']['model'] == Model:
                    lons_i = [
                        i[0] for i in faults[fi]['geometry']["coordinates"]
                    ]
                    lats_i = [
                        i[1] for i in faults[fi]['geometry']["coordinates"]
                    ]
                    dd = "sf"
