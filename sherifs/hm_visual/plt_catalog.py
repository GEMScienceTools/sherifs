...
def plt_catalog(do_catalog, Model_list, File_bg, catalog_file, Run_name, xmin,
                xmax, ymin, ymax, llcrnrlon, llcrnrlat, urcrnrlon, urcrnrlat,
    ...
    if do_catalog == True:
        ...
        for model in Model_list:
            file_catalog_rate_all = open(
                str(Run_name) +
                '/analysis/figures/catalogue/catalog_rates_all_' + model +
                '.txt', 'w')
                ...
                for line in lines_sub_area:
                    ...
                    if model == model_sub_area:
                        ...
                        for x1, y1 in zip(sub_area_lon_i, sub_area_lat_i
                                         ):  # creation du polygon de la zone
            ...
            for i in range(int(len(lines_of_the_file) / 2)):
                if len(lines_of_the_file[line_number].split('\t')) != 0:
                    ...
                    comp_value_i = lines_of_the_file[line_number +
                                                     1].split('\t')
                    if '\r\n' in comp_value_i:
                        comp_value_i.remove('\r\n')
                    if '\n' in comp_value_i:
                    ...
                    completeness_interpolate = interp1d(binning_comp,
                                                        comp_value)
                    completness_i = []
                ...
                for Yr in cat_Yr:  #loop on all the earthquakes in the catalogue to pick the magnitude
                    if cat_sig_Mw[index_cat] == 0:
                        ...
                        event_magnitude = np.random.triangular(
                            ...
                            cat_Mw[index_cat],
                            cat_Mw[index_cat] + cat_sig_Mw[index_cat] / 2.)
                        event_magnitude = np.random.normal(
                            loc=cat_Mw[index_cat], scale=cat_sig_Mw[index_cat])
    #                else :
    #                    event_magnitude = np.random.triangular(cat_sig_Mw[index_cat],cat_Mw[index_cat],cat_sig_Mw_plus[index_cat])
                ...
                for mag_i, completness_i in zip(bining_in_mag,
                                                completness_used):
                    ...
                    for Yr in cat_Yr:  #loop on all the earthquakes in the catalogue
                        ...
                        index_cat += 1
                    rate_cat[index_mag] = cat_model_bin[index_mag] / (
                            ...
                            int(
                                np.sum(
                                    ...
                                    [-(len(cat_sub_area_bin[index_sub_area]) -
                                       i):])))
                    number_of_earthquakes_for_rate_sub_area[
                        index_sub_area].append(number_of_earthquakes_for_rate_i)
                    index_sub_area += 1

                rate_cat_cum = []
                for i in range(len(rate_cat)):  #calculate the cumulative rate
                    rate_cat_cum.append(
                        np.sum(np.array(rate_cat)[-(len(rate_cat) - i):]))
            ...
            for index_mag in range(len(bining_in_mag)):
                ...
                if nb_eq <= 10:  #if there is less than 10 earthquakes, do the statistical représentativity
                    ...
                    unc_rate = (np.std(rate_cat_cum_density,
                                       axis=0)[index_mag]) / (np.square(
                                           mean_nb_eq_in_cat[index_mag]))
                    unc_rate = np.std(rates_simu)

                    rate_plus = np.array(rate_cat_cum_density).mean(
                        axis=0)[index_mag] + unc_rate
                    rate_minus = np.array(rate_cat_cum_density).mean(
                        axis=0)[index_mag] - unc_rate
                    if rate_minus < 0.:  # in order not to have negative values
                        rate_minus = np.array(rate_cat_cum_density).mean(
                            axis=0)[index_mag] / 100.

                    #if this is smaller than the percentils, take percentiles
                    if np.percentile(rate_cat_cum_density, 84,
                                     axis=0)[index_mag] > rate_plus:
                        rate_plus = np.percentile(rate_cat_cum_density,
                                                  84,
                                                  axis=0)[index_mag]
                    if np.percentile(rate_cat_cum_density, 16,
                                     axis=0)[index_mag] < rate_minus:
                        rate_minus = np.percentile(rate_cat_cum_density,
                                                   16,
                                                   axis=0)[index_mag]

                else:  #if there is more, use the percentiles of the distribution
                    rate_plus = np.percentile(rate_cat_cum_density, 84,
                                              axis=0)[index_mag]
                    rate_minus = np.percentile(rate_cat_cum_density, 16,
                                               axis=0)[index_mag]
                ...
                alpha = float(
                    np.count_nonzero(
                        rate_cat_cum_density,
                        axis=0)[index_mag]) / float(nb_sample) * 0.2
                alphas.append(alpha)
    ...
    #                    c='k', s=4, edgecolor='',marker = 'o',alpha = 0.8)
            ...
            plt.scatter(bining_in_mag,
                        np.array(rate_cat_cum_density).mean(axis=0),
                        c=rgba_colors,
                        s=20,
                        edgecolor='',
                        marker='s')
            ...
            plt.savefig(str(Run_name) +
                        '/analysis/figures/catalogue/catalogue_' + str(model) +
                        '.png',
                        dpi=180,
                        transparent=True)
            #plt.show()
            ...
            for poly_sub_area_i in bbPath_sub_areas:
                ...
                for index_mag in range(len(bining_in_mag)):
                    ...
                    if nb_eq <= 10:  #if there is less than 10 earthquakes, do the statistical représentativity
                        time_obs = int(
                            round(end_year_of_catalog -
                                  completness[0][index_mag]))
                        ...
                        rates_simu = []
                        for i in range(nb_sample):
                            yrs_start = random.choice(
                                range(time_simu - time_obs))
                            cat_sampled = cat[yrs_start:yrs_start + time_obs]
                            #print sum(cat_sampled), sum(cat_sampled)/float(time_obs)
                            rates_simu.append(
                                sum(cat_sampled) / float(time_obs))
                        ...
                        #if this is smaller than the percentils, take percentiles
                        if np.percentile(rate_in_sub_area_cum[index_sub_area],
                                         84,
                                         axis=0)[index_mag] > rate_plus:
                            rate_plus = np.percentile(
                                rate_in_sub_area_cum[index_sub_area],
                                84,
                                axis=0)[index_mag]
                        if np.percentile(rate_in_sub_area_cum[index_sub_area],
                                         16,
                                         axis=0)[index_mag] < rate_minus:
                            rate_minus = np.percentile(
                                rate_in_sub_area_cum[index_sub_area],
                                16,
                                axis=0)[index_mag]
                        ...
                        rate_plus = np.percentile(
                            rate_in_sub_area_cum[index_sub_area], 84,
                            axis=0)[index_mag]
                        rate_minus = np.percentile(
                            rate_in_sub_area_cum[index_sub_area], 16,
                            axis=0)[index_mag]
                ...
                plt.scatter(
                    bining_in_mag,
                    np.array(rate_in_sub_area_cum[index_sub_area]).mean(axis=0),
                    c=rgba_colors,
                    s=20,
                    edgecolor='',
                    marker='s')
                ...
                file_cat_sub_area = open(
                    str(Run_name) +
                    '/analysis/figures/catalogue/sub_area/mean_rate_cat_' +
                    sub_area_names[index_sub_area] + ' Model ' + str(model) +
                    '.txt', 'w')
                for index_mag in range(len(bining_in_mag)):
                    file_cat_sub_area.write(
                        str(bining_in_mag[index_mag]) + '\t' + str(
                            np.array(rate_in_sub_area_cum[index_sub_area]).mean(
                                axis=0)[index_mag]) + '\n')
            ...
            if mean_nb_eq_in_cat[0] > 3:
                catalog_for_map = np.genfromtxt(
                    str(Run_name) +
                    '/analysis/figures/catalogue/catalog_for_map_' +
                    str(model) + '.txt',
                ...
                for (Yr, M, lon, lat,
                     nb) in zip(yr_cat_for_map, M_cat_for_map, lon_cat_for_map,
                                lat_cat_for_map, nb_time_picked):
                    x, y = m(lon, lat)
                    # color scale
                    if M < 5.:
                        c = 'b'
...
#c = plt.cm.jet((M-min(M_cat_for_map))/(max(M_cat_for_map)-min(M_cat_for_map)))
                    ...
                    if nb >= float(
                            nb_inter
                    ) / 2.:  #plot the earthquake if it's piccked more than 50% of the iterations
                        m.plot(x,
                               y,
                               '.',
                               markersize=s,
        ...
        file_eq_rate_cat = open(
            str(Run_name) +
            '/analysis/figures/catalogue/cat_cumulative_eq_rate.txt', 'w')
        ...
        file_eq_rate_cat = str(
            Run_name) + '/analysis/figures/catalogue/cat_cumulative_eq_rate.txt'
