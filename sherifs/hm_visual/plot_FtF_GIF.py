...
def map_faults(Run_name, Model_list, scenarios_names_list, ScL_complet_list,
               BG_hyp_list, sample_list, b_value_list, MFD_type_list, llcrnrlon,
               llcrnrlat, urcrnrlon, urcrnrlat, File_bg, FileName_Prop,
               File_geom, plot_sr_use, visual_FtF, sub_area_file):
    nb_on_maps = False
    ...
    available_sets = read_input.extract_sc_input('input/' + Run_name +
                                                 '/ruptures.txt')
    ...
    for Model in Model_list:
        ...
        geom_scenar = Geometry_scenario.Geom_scenar(faults_names, File_geom,
                                                    Model)
        faults_lon = geom_scenar.faults_lon
        faults_lat = geom_scenar.faults_lat

        Column_Fault_name, Depths = geom.FaultGeometry(
            Model, File_geom)  #extract the geometries from the geometry file
        ...
        for Fault_name in faults_names:
            ...
            dip, oriented, upper_sismo_depth, lower_sismo_depth = geom.FaultProperties(
                FileName_Prop, Fault_name, Model)

            if depth[0] == 'sf':
                trace_lon, trace_lat, plot_trace, polygon = geom.get_sf_polygon(
                    lons, lats, upper_sismo_depth, lower_sismo_depth, dip,
                    oriented)
                ...
                poly_lons = np.concatenate(
                    [lon_top, np.array(list(reversed(lon_bottom)))])
                poly_lats = np.concatenate(
                    [lat_top, np.array(list(reversed(lat_bottom)))])
        ...
        if visual_FtF == True and use_basemap == True:
            for scenario_set in scenarios_names_list:
                ...
                if not os.path.exists(
                        str(Run_name) + '/analysis/figures/FtF/' + Model + '/' +
                        scenario_set):
                    os.makedirs(
                        str(Run_name) + '/analysis/figures/FtF/' + Model + '/' +
                        scenario_set)
...
#                                +  '/Log/Mmax_sample_1.txt')
                log_Mmax_file = (str(Run_name) + '/' + str(Model) +
                                 '/Log/Mmax_sample_' +
                                 str(ScL_complet_list[0]) + '_' + scenario_set +
                                 '_1.txt')

                sources_names, sources_Mmax, sources_Lengths, sources_Areas = Read_file.read_file_Mmax_log(
                    log_Mmax_file
                )  #read the log of Mmax associated with the model
                ...
                for rupt in rupture_set:
                    if len(Lon_bg) != 0:  #draw the background zone
                        maps_utils.draw_screen_poly(Lon_bg, Lat_bg, m, 'g', 0.2,
                                                    0.5, 'k')
                    m1 = copy.copy(m)
                    fault_colors = []
                    ...
                    figpath = str(
                        Run_name
                    ) + '/analysis/figures/FtF/' + Model + '/' + scenario_set + '/' + str(
                        index_rupt) + '.png'
                    maps_utils.make_fault_map(m1,
                                              fault_geom,
                                              fault_colors,
                                              figpath,
                                              title,
        ...
        if use_basemap == True:
            ...
            if len(Lon_bg) != 0:  #draw the background zone
                maps_utils.draw_screen_poly(Lon_bg, Lat_bg, m, 'g', 0.2, 0.5,
                                            'k')
            fault_colors = ['k' for fault in faults_names]

            title = 'Map of the model : ' + Model
            figpath = str(
                Run_name) + '/analysis/figures/FtF/' + Model + '/' + 'map.png'
            ...
            if os.path.exists(sub_area_file):
                ...
                for line in lines_sub_area:
                    ...
                    if model_sub_area in Model_list:
                        ...
                        maps_utils.draw_screen_poly(sub_area_lon_i,
                                                    sub_area_lat_i, m, 'k',
                                                    0.01, 0.1, 'k')
        ...
        mean_param = np.genfromtxt(
            Run_name + '/analysis/txt_files/mean_parameters_faults.txt',
            dtype=[('U100'), ('U100'), ('U1000'), ('f8'), ('f8')],
            delimiter='\t')
        model_mean_param = list(
            map(lambda i: mean_param[i][0], range(len(mean_param))))
        set_mean_param = list(
            map(lambda i: mean_param[i][1], range(len(mean_param))))
        ...
        Mmax_mean = list(map(lambda i: mean_param[i][4],
                             range(len(mean_param))))

        index_model = np.where(np.array(model_mean_param) == Model)[0]
        set_mean_param_model = np.take(set_mean_param, index_model)
        fault_name_mean_param_model = np.take(fault_name_mean_param,
                                              index_model)
        ...
        if use_basemap == True:
            ...
            figpath = str(
                Run_name
            ) + '/analysis/figures/FtF/' + Model + '/' + 'map_sliprate.png'
...
#            maps_utils.draw_screen_poly(Lon_bg, Lat_bg,  m ,'g' , 0.1, 0.5, 'k')
        ...
        for fault in faults_names:
            index_fault = np.where(
                np.array(fault_name_mean_param_model) == fault)[0][0]
        ...
        for scenario_set in scenarios_names_list:
            if use_basemap == True:
                ...
                figpath = str(
                    Run_name
                ) + '/analysis/figures/FtF/' + Model + '/' + 'map_Mmax_' + scenario_set + '.png'

            index_set = np.where(
                np.array(set_mean_param_model) == scenario_set)[0]
            fault_name_mean_param_set = np.take(fault_name_mean_param_model,
                                                index_set)
            ...
            for fault in faults_names:
                index_fault = np.where(
                    np.array(fault_name_mean_param_set) == fault)[0][0]
                Mmax = Mmax_mean_set[index_fault]

                cmap = matplotlib.cm.get_cmap('rainbow')
                rgba = cmap((float(Mmax) - min(Mmax_mean)) /
                            (max(Mmax_mean) - min(Mmax_mean)))
                fault_colors.append(rgba)
            if use_basemap == True:
                maps_utils.make_fault_map(m1,
                                          fault_geom,
        ...
        for scenario_set in scenarios_names_list:
            ...
            for MFD_type in MFD_type_list:
                if use_basemap == True:
                    ...
                    figpath = str(
                        Run_name
                    ) + '/analysis/figures/FtF/' + Model + '/' + 'map_NMS_' + MFD_type + '_' + str(
                        scenario_set) + '.png'
                slip_rep_data = np.genfromtxt(
                    Run_name + '/analysis/txt_files/slip_rep_on_faults_mean_' +
                    str(Model) + '_' + MFD_type + '_' + str(scenario_set) +
                    '.txt',
                    dtype=[('U100'), ('f8'), ('f8'), ('f8'), ('f8'), ('f8'),
                           ('f8'), ('f8'), ('f8'), ('f8'), ('f8'), ('f8'),
                           ('f8'), ('f8'), ('f8')],
                    delimiter='\t')
                fault_name_rep = list(
                    map(lambda i: slip_rep_data[i][0],
                        range(len(slip_rep_data))))
                p_NMS = list(
                    map(lambda i: slip_rep_data[i][14],
                        range(len(slip_rep_data))))
                ...
                if use_basemap == True:
                    ...
                    for fault in faults_names:
                        index_fault = np.where(
                            np.array(fault_name_rep) == fault)[0][0]
                ...
                if use_basemap == True:
                    ...
                    figpath = str(
                        Run_name
                    ) + '/analysis/figures/FtF/' + Model + '/' + 'map_seismicsliprate_' + MFD_type + '_' + str(
                        scenario_set) + '.png'
                    #                slip_rep_data = np.genfromtxt(Run_name + '/analysis/txt_files/slip_rep_on_faults_mean_'+str(Model)+'_'+ MFD_type +'_' +str(scenario_set)+'.txt',
                    #                           dtype = [('U100'),('f8'),('f8'),('f8'),('f8'),('f8'),('f8'),('f8'),
                    #                                    ('f8'),('f8'),('f8'),('f8'),('f8'),('f8'),('f8')], delimiter = '\t')
                    ...
                    for fault in faults_names:
                        ...
                        index_fault = np.where(
                            np.array(fault_name_mean_param) == fault)[0][0]
                ...
                for fault in faults_names:
                    ...
                    trace = []
                    for lon_i, lat_i in zip(fault_geom[id]["trace_lon"],
                                            fault_geom[id]["trace_lat"]):
                        trace.append([lon_i, lat_i])
                    trace = LineString(trace)

                    features.append(
                        Feature(geometry=trace,
                                properties={
                                    "id": id,
                                    "name": fault,
                                    "Mmax": Mmax,
                ...
                with open(
                        str(Run_name) + '/analysis/figures/FtF/' + Model + '/' +
                        MFD_type + '_' + str(scenario_set) + '.geojson',
                        'w') as f:
