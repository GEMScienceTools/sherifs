...
def geodetic_distance(lons1, lats1, lons2, lats2, diameter=2 * EARTH_RADIUS):
    ...
    distance = numpy.arcsin(
        numpy.sqrt(
            numpy.sin((lats1 - lats2) / 2.0)**2.0 + numpy.cos(lats1) *
            numpy.cos(lats2) * numpy.sin((lons1 - lons2) / 2.0)**2.0))
...
def average_azimuth(lons, lats):
    ...
    distances = geodetic_distance(lons[:-1], lats[:-1], lons[1:], lats[1:])
...
def point_at(lon, lat, azimuth, distance):
    ...
    dlon = numpy.arctan2(
        numpy.sin(tc) * sin_dists * cos_lat, cos_dists - sin_lat * sin_lats)
...
def orient_fault(lons, lats, orientated):
    ...
    compass_bearing = calculate_initial_compass_bearing((lats[0], lons[0]),
                                                        (lats[-1], lons[-1]))
...
def get_sf_polygon(lons, lats, upper_seismogenic_depth, lower_seismogenic_depth,
                   dip, oriented):
    """
    ...
    poly_lons = numpy.concatenate(
        [lon_top, numpy.array(list(reversed(lon_bottom)))])
    poly_lats = numpy.concatenate(
        [lat_top, numpy.array(list(reversed(lat_bottom)))])
...
def FaultProperties(File_prop, Name_of_fault, Model):
    ...
    if not ".geojson" in File_prop:
        ...
        index_fault = numpy.where(
            numpy.array(Column_fault_name[index_model[0]:index_model[-1] +
                                          1]) == Name_of_fault)
...
def FaultGeometry(Model, File_geom):
    ...
    if not ".geojson" in File_geom:
        ...
        InfosZonage = numpy.genfromtxt(NomFichier_InfosZonage,
                                       dtype=[('U100'), ('U100'), ('f8'),
                                              ('f8'), ('U100')],
                                       skip_header=1)
        ...
        for cle in DicoZone.keys():
            indices_ZonesSelec = numpy.where(
                numpy.array(Column_Fault_name_tmp) == cle)
            ColonneNomZone_inter = numpy.take(Column_Fault_name_tmp,
                                              indices_ZonesSelec)
