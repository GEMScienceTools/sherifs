...
def read_file_sr_log(log_sr_file):
    try:
        ...
        log_file = np.genfromtxt(log_sr_file,
                                 dtype=[('U100'), ('f8')],
                                 skip_footer=1)
    faults_names = list(map(lambda i: log_file[i][0], range(len(log_file))))
    faults_slip_rates = list(map(lambda i: log_file[i][1],
                                 range(len(log_file))))
...
def read_file_as_log(log_as_file):
    log_file = np.genfromtxt(log_as_file,
                             dtype=[('U100'), ('f8'), ('U100')],
                             delimiter='\t')
    try:
        a_s = log_file[2][1]
...
def read_file_general_param_log(log_general_param_file):
    log_file = np.genfromtxt(log_general_param_file,
                             dtype=[('U100'), ('f8')],
                             delimiter='\t')
...
def read_file_mfd_log(log_mfd_file):
    #print log_mfd_file
    log_file = np.genfromtxt(log_mfd_file,
                             dtype=[('U1000'), ('f8'), ('U1000')],
                             delimiter='\t')
    sources_names = list(map(lambda i: log_file[i][0], range(len(log_file))))
