...
def checking_the_input(input_file):
    ...
    OQ_job = OQ_job_Creator(
        Run_name)  # ask the info about the run and create the job.ini file
    Mmin = OQ_job.Mmin
    ...
    time_i = time.time()
    (mega_MFD, df_mega_MFD, scenarios_names_list, ScL_complet_list, ScL_list,
     Model_list, BG_hyp_list, dimension_used_list, faults_name_list,
     sample_list, b_value_list, MFD_type_list, m_Mmax, mega_bining_in_mag,
     a_s_model, b_sample, sm_sample, Mt_sample, sources_Lengths, sources_Areas,
     logictree) = Extract_data.extract(Run_name)
    ...
    if plot_Mmax == True:
        ...
        plt.savefig(str(Run_name) + '/analysis/figures/Mmax/Hist_Mmax.png',
                    dpi=180)
        plt.close()
    ...
    if plot_Mmax == True:
        ...
        for Model in Model_list:
            for scenario in scenarios_names_list:
                log_Mmax_file = (str(Run_name) + '/' + str(Model) + '/Log/' +
                                 'Mmax_sample_' + ScL_complet_list[0] + '_sc_' +
                                 str(scenario) + '_1.txt')
                log_file = np.genfromtxt(log_Mmax_file,
                                         dtype=[('U10000'), ('f8'), ('U100'),
                                                ('f8'), ('f8'), ('f8')],
                                         delimiter='\t')
                sources_names = list(
                ...
                for fault in faults_name_list[0]:
                    ...
                    if not os.path.exists(
                            str(Run_name) + '/analysis/figures/Mmax/' + Model):
                        os.makedirs(
                            str(Run_name) + '/analysis/figures/Mmax/' + Model)
                    if not os.path.exists(
                            str(Run_name) + '/analysis/figures/Mmax/' + Model +
                        ...
                        os.makedirs(
                            str(Run_name) + '/analysis/figures/Mmax/' + Model +
                            '/' + scenario)
                    plt.savefig(str(Run_name) + '/analysis/figures/Mmax/' +
                                Model + '/' + scenario + '/' + fault + '.png',
                                dpi=80)
                    plt.close()
            i_model += 1
    '''########################
    ...
    plt.savefig(str(Run_name) + '/analysis/figures/Mmax/Mmax_vs_NMS.png',
                dpi=180)
    ...
    try:
        (seismological_moment_rate, catalog_cum_rate, yr_cat_for_map,
         M_cat_for_map,
         lon_cat_for_map, lat_cat_for_map) = plt_catalog.plt_catalog(
             do_catalog, Model_list, File_bg, catalog_file, Run_name, xmin,
        ...
        seismological_moment_rate, catalog_cum_rate = plt_catalog.plt_catalog(
            do_catalog, Model_list, File_bg, catalog_file, Run_name, xmin, xmax,
            ymin, ymax, llcrnrlon, llcrnrlat, urcrnrlon, urcrnrlat,
            completness_file, nb_inter, bining_in_mag, end_year_of_catalog,
            sub_area_file)
    ...
    (total_list_ScL, total_list_dimension_used, geologic_moment_rate,
     ...
     mega_mfd_cummulative, total_list_model,
     total_list_sample, total_list_BG_hyp) = Plot_mfd.plt_mfd(
         Run_name, mega_MFD, scenarios_names_list, ScL_complet_list, ScL_list,
         Model_list, BG_hyp_list, dimension_used_list, faults_name_list,
         sample_list, b_value_list, MFD_type_list, m_Mmax, mega_bining_in_mag,
         a_s_model, b_sample, sm_sample, Mt_sample, plot_mfd, plot_as_rep,
         plot_Mmax, xmin, xmax, ymin, ymax, catalog_cum_rate,
         plot_mfd_detailled, bining_in_mag)
    print('\nTime to plot the MFDs : ' + str(round(time.time() - time_i, 2)) +
          ' s.\n')
    ...
    if plot_sr_use == True:
        time_i = time.time()
        slip_rate_rep.sr_rate(Run_name, scenarios_names_list, mega_MFD,
                              Model_list, MFD_type_list, sub_area_file,
                              File_geom, FileName_Prop)
    ...
    if plot_rup_freq == True:
        Participation_rates.plt_EQ_rates(
            ...
            dimension_used_list, faults_name_list, sample_list, b_value_list,
            MFD_type_list, m_Mmax, mega_bining_in_mag, a_s_model, b_sample,
            sm_sample, Mt_sample, plot_mfd, plot_as_rep, plot_Mmax, xmin, xmax,
            ymin, ymax, file_faults_data, File_bg, File_geom, sub_area_file,
            FileName_Prop)
#            #extract the faults data
    ...
    if test_fit_to_data == True:
        Sampling_analysis.sampling_analysis(
            Run_name, Model_list, m_Mmax, b_sample, a_s_model,
    ...
    plot_FtF_GIF.map_faults(Run_name, Model_list, scenarios_names_list,
                            ScL_complet_list, BG_hyp_list, sample_list,
                            b_value_list, MFD_type_list, llcrnrlon, llcrnrlat,
                            urcrnrlon, urcrnrlat, File_bg, FileName_Prop,
    ...
    ###############################
    ##############################'''
    moment_rate.moment_rate(Run_name, plot_moment_rate,
                            geologic_moment_rate_no_as, geologic_moment_rate,
                            seismological_moment_rate, scenarios_names_list,
                            total_list_scenario_name, MFD_type_list,
                            total_list_MFD_type)
    '''#############################
    ###############################
    #     CREATE PFD REPORT
...
def main(argv):
    ...
    p.arg(name='input_file',
          help='.txt file with the information concerning the run.')
