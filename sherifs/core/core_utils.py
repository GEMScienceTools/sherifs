...
def progress(model_MFD, calculation_log, ratio_done, print_percent, rup_rates,
             fault_prop, bin_mag):
...
def weight_fault_sampling(picked_bin, rup_in_bin, faults_names,
                          faults_slip_rates, slip_rate_use_per_fault,
                          faults_alone, scenarios_names, faults_isolated,
                          index_faults_in_scenario, rup_rates, empty_rups):
        ...
        if str(i_rup) in empty_rups:  #the rup is empty
            ...
            if len(involved_faults) == 0:  #it's a fault
                ...
                if 1. - float(sr_used) / float(sr0) >= 0.:
                    if involved_faults[
                            0] in faults_alone:  #we give a boost for faults that are alone so they can  break more
                        weight_i = 4. * 5.
                        #weight_i = 2. * (sr0 - sr_used)**2.
                        weight_fault.append(weight_i)
                    elif involved_faults[
                            0] in faults_isolated:  #we give a boost for faults that are isolated so they can  break more
                        if (float(sr_used) / float(sr0)) < 0.2:
                            weight_i = 4. * srfactor
                        else:
                            weight_i = (4. - 4. *
                                        (float(sr_used) / float(sr0) + 0.3)**6.
                                       ) * srfactor
                        if weight_i < 1.:
                            weight_i = 1.
                        weight_fault.append(weight_i)
                ...
                for index in involved_faults:
                    ...
                    if float(sr0) / float(max(faults_slip_rates)) <= 0.2:
                        ...
                        srfactor = 5. * float(sr0) / float(
                            max(faults_slip_rates))
...
def variable_spending(index_fault, M_slip_repartition, faults_budget,
                      slip_rate_use_per_fault, size_of_increment,
                      faults_slip_rates, picked_rup, faults_names,
                      sum_fault_budget):
            ...
            if not area_i["properties"]["b_value"] == None:
                ...
                if apply_global_bg == True:
                    ...
                    if type(area_i["properties"]["bg"]) == type([0, 0]):
                        ...
                        print(
                            "ERROR !!! please verify the local background proportions"
                        )
                ...
                for rup_i in rup_rates:
                    ...
                    for id_s in id_sections:
                        is_in = False
                        for lon_i, lat_i in zip(faults_lon[id_s],
                                                faults_lat[id_s]):
                            if is_in == False:
                        ...
                        if is_in == True:
                            nb_in += 1
                    if nb_in != 0:
                        associated_rup_i.append(
                            rup_rates.get(rup_i).get("rup_id"))
                        associated_weight_i.append(
                            float(nb_in) / float(nb_sections))
...
def check_local_mfd(rup_rates, rup_in_bin, picked_bin, bin_mag, local_mfds,
    ...
    for local_mfd, associated_rup_i, associated_weight_i in zip(
            local_mfds, associated_rup, associated_weight):
        ...
        if sum(rates) != 0.:
            p_rates = (rates) / sum(
                rates)  # transform to a probability distribution.
            ...
            min_acceptable = local_mfd[
                picked_bin] - local_mfd[picked_bin] * wiggle_room
            max_acceptable = local_mfd[
                picked_bin] + local_mfd[picked_bin] * wiggle_room
