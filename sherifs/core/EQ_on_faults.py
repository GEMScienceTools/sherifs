...
class EQ_on_faults_from_sr():
    ...
    def __init__(self, Run_Name, M_min, mfd_param, faults_names, faults_area,
                 faults_length, faults_width, faults_slip_rates, scenarios,
                 faults_shear_mod, path, pathlog, sample, selected_ScL,
                 dimention_used, use_all_ScL_data, faults_mecanism, bg_ratio,
                 size_of_increment, mfd_hyp, count_reruns, faults_lon,
                 faults_lat, Mmax_range, calculation_log_file, branch, param):
    ...
    def initialize(self):
        ...
        log_calculation_file = open(
            self.pathlog + '/calculation_sample_' + str(self.sample) + '.txt',
            'w')

        log_sliprep_file = self.pathlog + '/sliprep_sample_' + str(
            self.sample) + '.pkl'
        ...
        f_mmax = run_name + '/LOG/' + model_name + '_mmax_' + scl_name + '_' + set_name + '_' + str(
            self.sample) + '.pkl'
        ...
        if re_use == False:
            ...
            ]  #contains the indexs of the faults in each scenario
            for scenario in scenarios_names:
                i_scenario = []
                for i in range(len(scenario)):
                    i_scenario = i_scenario + list(
                        np.where(
                            np.array(faults_names) == scenario["f_%s" %
                                                               str(i + 1)]))
            ...
            for indexes_i in index_faults_in_scenario:
                ...
                for index_fault in indexes_i[0]:
                    list_i = str(self.faults_lon[index_fault][0]) + str(
                        self.faults_lon[index_fault][-1]) + str(
                            self.faults_lat[index_fault][0]) + str(
                                self.faults_lat[index_fault][-1])
            ...
            while Mmax < Mmaxmin or Mmax > Mmaxmax:
                ...
                log_Mmax_file = open(
                    run_name + '/LOG/' + model_name + '_Log_Mmax_sample_' +
                    scl_name + '_' + set_name + '_' + str(self.sample) + '.txt',
                    'w')

                if loop_Mmax == 1:
                    Mmaxs = scaling_laws.Calc_Mmax(
                        f_area_ar, scenario_area, f_len_ar, scenario_length,
                        f_width_ar, scenario_width, self.selected_ScL,
                        self.dimention_used, self.use_all_ScL_data,
                        self.faults_mecanism, index_faults_in_scenario,
                        self.sample)
                else:
                    Mmaxs = scaling_laws.Calc_Mmax(
                        f_area_ar, scenario_area, f_len_ar, scenario_length,
                        f_width_ar, scenario_width, self.selected_ScL,
                        self.dimention_used, self.use_all_ScL_data,
                        self.faults_mecanism, index_faults_in_scenario, 10)
                ...
                for Mmax_i in Mmax_faults:
                    line = (
                        faults_names[index_fault] + '\t' +
                        str(round(faults_length[index_fault] / 1000., 1)) +
                        '\t' + final_fault_length[index_fault] + '\t' +
                        str(round(self.faults_width[index_fault], 1)) + '\t' +
                        str(round(self.faults_area[index_fault] / 1000000.,
                                  1)) + '\t' + str(Mmax_i) + '\n')
                ...
                for Mmax_i in Mmax_scenario:
                    line = (
                        str(scenarios_names[index_scenario]) + '\t' +
                        str(round(scenario_length[index_scenario] / 1000., 1)) +
                        '\t' + final_scenario_length[index_scenario] + '\t' +
                        str(round(scenario_width[index_scenario], 1)) + '\t' +
                        str(round(scenario_area[index_scenario] / 1000000.,
                                  1)) + '\t' + str(Mmax_i) + '\n')
                    log_Mmax_file.write(line)
                    index_scenario += 1
                log_Mmax_file.close()
                ...
                if loop_Mmax == 30:
                    print(
                        'An Mmax incompatible with the ruptures is imposed!! Change it in run.info or change the rupture.txt file'
                    )

            if loop_Mmax >= 3:
                print('Mmax imposed: ' +
                      str(Mmax))  #+'   see EQ_on_faults.py for details.')
            ...
            with open(f_mmax, 'wb') as f:
                ...
                dump_to_file = [
                    Mmax_faults, Mmax_scenario, final_fault_length,
                    final_scenario_length, faults_alone, faults_isolated,
                    index_faults_in_scenario, scenario_area
                ]
        ...
        ##################################################################'''
        bin_mag = np.linspace(M_min, Mmax,
                              int(round((Mmax - M_min), 1) * 10. + 1))
        '''#####################################################################
        ...
        if np.size(scenarios_names) != 0:
            for j in range(len(scenarios_names)):
                rup_rates.update({
                    str(i_end + j): {
                        'rup_id': i_end + j,
                        'rup_name': scenarios_names[j],
                        'involved_faults': index_faults_in_scenario[j][0],
                        'Mmax': Mmax_scenario[j],
                        'area': scenario_area[j],
                        'rates': np.zeros(len(bin_mag))
                    }
                })
        ...
        if str(self.sample) == '1':
            ...
            for i in range(len(rup_rates)):
                log_rup_file.write(
                    str(rup_rates.get(str(i)).get('rup_id')) + '\t')
        ...
        f_bin_pop = run_name + '/LOG/' + model_name + '_bin_pop_' + scl_name + '_' + set_name + '_' + str(
            self.sample) + '.pkl'
        ...
        for fault_area in faults_areas:  #adding the moment rate of each fault
            Total_moment_faults_rate_init += faults_shear_mod[
                index_fault] * fault_area * faults_slip_rates[index_fault]
            index_fault += 1

        log_calculation_file.write(
            'Moment_rate_fault_initial calculated using muAs' + '\t' +
            str(Total_moment_faults_rate_init) + '\t' + 'N.m' + '\n')
        ...
        for mag in bin_mag:
            M0 = mag_to_M0(mag)
            p_MFD_MO.append(
                fault_prop(mag) * p_MFD[index_mag] *
                M0)  #considering the proportion that needs to be in the BG
            index_mag += 1
        ...
        index_mag = 0
        for mag in bin_mag:
            moment_rate_fault_bin = p_MFD_MO[
                index_mag] * Total_moment_faults_rate_init  # M0 of the faults for that bin
            moment_rate_BG_bin = (
                (1 - fault_prop(mag)) * moment_rate_fault_bin) / fault_prop(
                    mag)  # M0 of the background for that bin
        ...
        ##################################################################'''
        size_of_increment = float(
            self.size_of_increment) * 0.001  #in meter/year
        if self.count_reruns != 1:
            size_of_increment = size_of_increment / (
                float(self.count_reruns) * 1.5 - 1.
            )  #divide the size of the increment if there are re-runs

        faults_budget = {}
        #        min_budget = 10**10
        for index_fault in range(len(faults_names)):
            nb_dsr = int(
                round(faults_slip_rates[index_fault] / size_of_increment,
                      0))  #nb of dsr to spend
            if nb_dsr == 0:
                nb_dsr = 1
...
#                min_budget = nb_dsr
        ...
        moment_rate_in_bin = np.zeros(
            len(bin_mag))  # moment aleardy present in the bin of magnitude
        model_MFD = []
        ...
        bin_target_reached = []
        len_faults_budget = [
        ]  # will be use to check if the calculation is stuck and too slow (see under)
        aseismic_count = 0
        ...
        faster_rup_weight = self.param["main"]["parameters"][
            "faster_rup_weight"]
        ...
        if local_MFD in ["True", "true"]:
            ...
            local_mfds, associated_rup, associated_weight = core_utils.link_rup_mfd_area(
                rup_rates, f_mfd_area, self.faults_lon, self.faults_lat,
                bin_mag, self.bg_ratio)
        ...
        if faster_rup_weight == True:
            nb_weigthings_rup_sampling = int(
                self.param["main"]["parameters"]["nb_weigthings_rup_sampling"])
            if "type_weigthings_rup_sampling" in self.param["main"][
                    "parameters"].keys():
                type_weigthings_rup_sampling = self.param["main"]["parameters"][
                    "type_weigthings_rup_sampling"]
            else:
                type_weigthings_rup_sampling = "lin"
            if type_weigthings_rup_sampling == "log":
                weigthing_built = [
                    int(i) for i in np.logspace(0., np.log10(budget_init),
                                                nb_weigthings_rup_sampling)
                ]
            if type_weigthings_rup_sampling == "lin":
                weigthing_built = [
                    int(i) for i in np.linspace(0., budget_init,
                                                nb_weigthings_rup_sampling)
                ]
        else:
        ...
        while sum_fault_budget != 0:  # sum(faults_budget.values()) as long as there is some slip-rate to spend we keep going
            ...
            if ratio_done > 0.01:
                model_MFD, self.calculation_log_file, print_percent = core_utils.progress(
                    model_MFD, self.calculation_log_file, ratio_done,
                    print_percent, rup_rates, fault_prop, bin_mag)
            ...
            if len(empty_bins) != len(bin_mag):
                ...
                tmp = time.time()
                target_i = target.get_new_target(number_of_loops,
                                                 moment_rate_in_bin, p_MFD_MO,
                                                 target_moment_per_bin, bin_mag,
                ...
                if sum(target_i) == 0.:
                    target_i = p_MFD_MO
                try:
                    most_likely_pick.append(bin_mag[list(target_i).index(
                        max(list(target_i)))])
                except:
                    print(target_i)
                    most_likely_pick.append(bin_mag[list(target_i).index(
                ...
                if deep_analysis == True:
                    percent = round(
                        (1. - float(sum_fault_budget) / float(nb_ss_to_spend)) *
                        100.)
                    ...
                    if percent != old_percent:
                        ...
                        print("budget left : ", sum_fault_budget, " | ",
                              percent, "%")
                        time_str = core_utils.seconds_to_str(
                            time_target_building)
                        ...
                        print("time cleaning weighting rupture weigth : ",
                              time_str)
                        time_str = core_utils.seconds_to_str(
                        ...
                        time_str = core_utils.seconds_to_str(time.time() -
                                                             inti_core_time)
                        print("total core time : ", time_str)
                        tot_core_time = time.time() - inti_core_time
                        unaccounted_t = tot_core_time - (
                            time_target_building + time_weight_rupt +
                            time_clean_w_rupt + time_checking_target_reach +
                            time_checking_empty_bin +
                            time_checking_empty_faults + time_spending_dsr)
                        time_str = core_utils.seconds_to_str(unaccounted_t)
                        print("unaccounted time : ", time_str)
                        print("max target : ", round(max(target_i), 4),
                              "| last bin w : ", str(round(target_i[-1], 6)))
                        print("Empty mag bins:")
                        print(empty_bins)
                        budget_last_bin = 0
                        for rup_i in rup_in_bin[-1]:
                            if not str(rup_i) in empty_rups:
                                for f_i in rup_rates.get(
                                        str(rup_i)).get('involved_faults'):
                        ...
                        ax1.plot(bin_mag,
                                 target_moment_per_bin,
                                 label=("target_moment_per_bin"),
                                 marker='x')
                        ax1.plot(bin_mag,
                                 moment_rate_in_bin,
                                 label=("moment_rate_in_bin"),
                                 marker='x')
                        ax1.set_yscale('log')
                        ax1.legend()

                        plt.savefig(self.pathlog + '/Log_tmp_' +
                                    str(self.sample) + '.png',
                                    dpi=80,
                                    transparent=True)
                        plt.close()
                ...
                if not picked_bin in empty_bins:
                    ...
                    if do_rup_weight == True:
                        ...
                        for index_mag in list_of_bins:
                            if index_mag in empty_bins:
                                ...
                                weight_rup_i = core_utils.weight_fault_sampling(
                                    index_mag, rup_in_bin, faults_names,
                                    faults_slip_rates, slip_rate_use_per_fault,
                                    faults_alone, scenarios_names,
                                    faults_isolated, index_faults_in_scenario,
                                ...
                                if local_MFD == True:
                                    ...
                                    factor_on_weight = core_utils.check_local_mfd(
                                        rup_rates, rup_in_bin[index_mag],
                                        index_mag, bin_mag, local_mfds,
                                        associated_rup, associated_weight)
                                    weight_rup_i = np.array([
                                        i * w for i, w in zip(
                                            weight_rup_i, factor_on_weight)
                                    ])
                                    weight_rup_i /= weight_rup_i.sum()

                                if sum(weight_rup_i) != 0.:
                                    weight_rup_i = [
                                        float(i) / sum(weight_rup_i)
                                        for i in weight_rup_i
                                    ]
                        ...
                        for index_mag in list_of_bins:
                            if not index_mag in empty_bins:
                                ...
                                if (set(rup_in_bin[index_mag]) &
                                        set(empty_rups)):
                                    i = 0
                                    for i_rup in rup_in_bin[index_mag]:
                                        if str(
                                                i_rup
                                        ) in empty_rups:  #the rup is empty
                                            weight_rup[i] = 0.
                                        i += 1
                    ...
                    try:
                        ...
                        i_picked = np.where(
                            np.random.multinomial(1, weight_rup) == 1)[0][0]
                    ...
                    index_fault = rup_rates.get(
                        str(picked_rup)).get('involved_faults')
                    if bool_target_set == False:
                        ...
                        if last_bins_empty == True:  #if all the slip_rate has been spared for the Mmax - 0.3
                            rate_tot_model = rates.get_rate_model(
                                rup_rates, fault_prop, bin_mag)
                        ...
                        if moment_rate_left <= (1. -
                                                0.00001) * moment_rate_required:
                            #if not enough momen rate left, stop spending on the larger EQs
                            self.calculation_log_file.write(
                                '\n Not enough moment left ')
                            for rup_i in rup_in_bin[-3] + rup_in_bin[
                                    -2] + rup_in_bin[-1]:
                                if not str(rup_i) in empty_rups:
...
#                                    antelast_bins_empty = False
                        if antelast_bins_empty == True and bool_target_set == False:
                            if moment_rate_in_bin[-3] >= 2. * (
                                    moment_rate_in_bin[-2] +
                                    moment_rate_in_bin[-1]):
                                self.calculation_log_file.write(
                                ...
                                print(
                                    "set target - antepenultimate bin getting too high"
                                )
                        ...
                        if number_of_loops > number_of_loops_last_checked + 50.:
                            ...
                            moment_rate_required = 0.
                            for index_mag in range(
                                    len(bin_mag) - 3
                            ):  #loop on all the magnitude bins except the three last ones
                                rate_Mi_check = rate_tot_model[index_mag]
                                target_GR_i_check = rate_Mmax_check * p_MFD[
                                    index_mag] / p_MFD[-2]
                                #difference between the target at this point and what is already filled in terms of moment rate
                                moment_rate_required += (
                                    (mag_to_M0(bin_mag[index_mag]) *
                                     target_GR_i_check) -
                                    (mag_to_M0(bin_mag[index_mag]) *
                                     rate_Mi_check)) * fault_prop(
                                         bin_mag[picked_bin])
                        ...
                        if do_the_target == True and bool_target_set == True:
                            ...
                            for t_mag_bin in range(len(bin_mag)):
                                TARGET.append(
                                    rate_Mmax * p_MFD[t_mag_bin] / p_MFD[-2]
                                )  #target of the picked bin in order to fit the original distribution
                    ...
                    if sr_to_spend == True:
                        ...
                        if option_fast == True:
                            min_budget_local = min(
                                [faults_budget[i] for i in index_fault])
                            nb_loop_spending = int(min_budget_local /
                                                   min_budget)
                        ...
                        if bool_target_set == True:
                            ...
                            target_mfd_i = TARGET[picked_bin] * fault_prop(
                                bin_mag[picked_bin]
                            )  #target of the picked bin in order to fit the original distribution

                            if rate_Mi < target_mfd_i:  #for this bin, the target hasn't been reached yet

                                if uniform_spending == True or len(
                                        index_fault) == 1:
                                    moment_rate_i = 0.
                                    for index in index_fault:
                                        for loop_spending in range(
                                                nb_loop_spending):
                                            #M_slip_repartition[index].append(picked_rup)
                                            M_slip_repartition[str(
                                                faults_names[index])][str(
                                                    picked_rup)] += 1
                                        faults_budget[
                                            index] += -1 * nb_loop_spending
                                        sum_fault_budget += -1 * nb_loop_spending
                                        slip_rate_use_per_fault[
                                            index] += size_of_increment * nb_loop_spending
                                    rup_rates[str(picked_rup)]['rates'][
                                        picked_bin] += rate_i * nb_loop_spending
                                    rate_in_model[
                                        picked_bin] += rate_i * nb_loop_spending
                                    moment_rate_i += mag_to_M0(
                                        mag) * rate_i * nb_loop_spending
                                else:
                                    moment_rate_i = 0.
                                    for loop_spending in range(
                                            nb_loop_spending):
                                        M_slip_repartition, faults_budget, slip_rate_use_per_fault, nb_sdr_used, sum_fault_budget = core_utils.variable_spending(
                                            index_fault, M_slip_repartition,
                                            faults_budget,
                                            slip_rate_use_per_fault,
                                            size_of_increment,
                                            faults_slip_rates, picked_rup,
                                            faults_names, sum_fault_budget)
                                        #adding to the rate
                                        rup_rates[str(picked_rup)]['rates'][
                                            picked_bin] += rate_i * (
                                                nb_sdr_used)
                                        rate_in_model[picked_bin] += rate_i * (
                                            nb_sdr_used)
                                        moment_rate_i += mag_to_M0(
                                            mag) * rate_i * (nb_sdr_used)

                                #substracting the moment used from the target
                                moment_rate_in_bin[
                                    picked_bin] += moment_rate_i  #* ( 2.-fault_prop(mag))
                            ...
                            if uniform_spending == True or len(
                                ...
                                for loop_spending in range(nb_loop_spending):
                                    M_slip_repartition, faults_budget, slip_rate_use_per_fault, nb_sdr_used, sum_fault_budget = core_utils.variable_spending(
                                        index_fault, M_slip_repartition,
                                        faults_budget, slip_rate_use_per_fault,
                                        size_of_increment, faults_slip_rates,
                                        picked_rup, faults_names,
                                        sum_fault_budget)
                                    #adding to the rate
                                    rup_rates[str(picked_rup)]['rates'][
                                        picked_bin] += rate_i * (nb_sdr_used)
                                    rate_in_model[picked_bin] += rate_i * (
                                        nb_sdr_used)
...
#                    number_of_loops_before = number_of_loops
                        ...
                        if not index_mag in empty_bins:
                            ...
                            for i_rup in rup_in_bin[index_mag]:
                                if nb_rup_not_empty == 0:
                                    rup_is_empty = False
                                    for index_fault in rup_rates.get(
                                            str(i_rup)).get('involved_faults'):
                ...
                if len(len_faults_budget) > 3:
                    ...
                    if number_of_loops_for_nothing_before < number_of_loops_for_nothing - 100:
                        ...
                        if len_faults_budget[-1] == len_faults_budget[-10]:
                            ...
                            rup_still_used = [
                                i for i in rup_in_bin[picked_bin]
                                if not i in empty_rups
                            ]
                            fault_still_used = []
                            for rup_i in rup_still_used:
                                fault_still_used += list(
                                    rup_rates.get(
                                        str(rup_i)).get('involved_faults'))
                            fault_still_used = set(fault_still_used)
                            for fault, index_fault in zip(
                                    faults_names, range(len(faults_names))):
                                if not (index_fault in fault_still_used) and (
                                        faults_budget[index_fault] > 0):
                ...
                while sum_fault_budget != 0:  # as long as there is some slip-rate to spend we keep going
                    ...
                    for index_fault in range(len(faults_names)):
                        if faults_budget[index_fault] > 0:
                            ...
                            #                            M_slip_repartition[index_fault].append('NMS')
                            M_slip_repartition[str(
                                faults_names[index_fault])]['NMS'] += 1
                            aseismic_count += 1
        ...
        if TARGET == []:
            ...
            rate_tot_model = rates.get_rate_model(rup_rates, fault_prop,
                                                  bin_mag)
        ...
        #
        ##################################################################'''
        rate_f_in_model, rate_bg_in_model = rates.get_rate_faults_n_bg(
            rup_rates, fault_prop, bin_mag)
        ...
        plt.yscale('log')
        plt.savefig(self.pathlog + '/Log_target_fit_' + str(self.sample) +
                    '.png',
                    dpi=180,
                    transparent=True)
        plt.close()
        ...
        while bin_mag[index_mag] < 5.:
            index_5 += 1
        self.ratio_test = np.mean([
            abs(TARGET[index_5 + 0] / rate_tot_model[index_5 + 0]),
        ...
        for index_mag in range(len(bin_mag)):
            array_absolute_ratio.append(
                abs(MFD_to_test[index_mag] / target_to_test[index_mag]))
        self.ratio_test = np.mean(array_absolute_ratio)
        print('ratio between the target and the shape of the model : ' +
              str(round(self.ratio_test, 2)))
        self.calculation_log_file.write(
            '\nratio between the target and the shape of the model : ' +
            str(round(self.ratio_test, 2)))
        '''##################################################################
        #
        ...
        if np.size(scenarios_names) != 0:
            for j in range(len(scenarios_names)):
                self.OQ_entry_scenarios.append(
                    rup_rates.get(str(i_end + j)).get('rates'))
        self.faults_names = faults_names
        ...
        log_calculation_file.write(
            'Moment_rate_faults_final calculated using 10^(1.5M+9.1)' + '\t' +
            str(Total_moment_rate_fault) + '\t' + 'N.m' + '\n')
        log_calculation_file.write(
            'Ratio of NMS slip rate counted in the iterative process: ' + '\t' +
            str(round((100.) * (aseismic_count / nb_ss_to_spend))) + '\t %\n')
        log_calculation_file.write(
            'Moment_rate_model calculated using 10^(1.5M+9.1)' + '\t' +
            str(Total_moment_rate_fault) + '\t' + 'N.m' + '\n')
        log_calculation_file.close()
        self.ratio_NMS = round(
            (100.) *
            (1. - Total_moment_rate_fault / Total_moment_faults_rate_init))
        print('ratio of NMS : ' +
              str(round((100.) * (aseismic_count / nb_ss_to_spend))))
        self.calculation_log_file.write(
            '\nratio of NMS : ' +
