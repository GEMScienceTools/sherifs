...
def get_new_target(number_of_loops, moment_rate_in_bin, p_MFD_MO,
                   target_moment_per_bin, bin_mag, empty_bins,
                   bin_target_reached, rup_in_bin):
    ...
    target_i = p_MFD_MO * (
        ((target_moment_per_bin - moment_rate_in_bin) /
         (target_moment_per_bin)) / len(bin_mag)
    )  #**3   # initial target - moment already present in the bin

    target_i = (target_i) / sum(
        target_i
    )  # normalize the target to use it as a probability distribution
    for i in range(len(target_i)):
        if 1.05 * p_MFD_MO[i] >= shape_mfd_i[
                i]:  # if too low, increase the probability
            target_i[i] = target_i[i] * 100.
        if 1.05 * p_MFD_MO[i] <= shape_mfd_i[
                i]:  # if too high, reduce the probability
            target_i[i] = target_i[i] / 100.
        if target_i[i] <= 0.:
            target_i[
                i] = 0.  # p_MFD_MO[i] / 1000. #can't have a negative number in the distribution so we put a very small one (10E-15 N.m)
        if i in empty_bins:  # or len(rup_in_bin[i]) == 0 :
            target_i[
                i] = 0.  #p_MFD_MO[i] / 100000.  # don't pick bins where target is reached
...
def get_new_target_v2(number_of_loops, moment_rate_in_bin, p_MFD_MO,
                      target_moment_per_bin, bin_mag, empty_bins,
                      bin_target_reached, rup_in_bin):
    if number_of_loops > 200:
        shape_mfd_i = (moment_rate_in_bin) / sum(moment_rate_in_bin)
        ...
        for i in range(len(bin_mag)):
            ...
            if i in bin_target_reached:
                ...
                target_i[
                    i] = 0.  # can't have a negative number in the distribution
            else:
                target_i[i] = p_MFD_MO[i]
    else:
        target_i = p_MFD_MO
