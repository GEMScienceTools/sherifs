...
def GR(mfd_param, bin_mag):
    ...
    for mag in bin_mag:
        p_i = (((np.exp(-Beta * (mag - 0.05 - bin_mag[0]))) /
                (1. - np.exp(-Beta * (bin_mag[-1] - bin_mag[0])))) -
               ((np.exp(-Beta * (mag + 0.05 - bin_mag[0]))) /
                (1. - np.exp(-Beta * (bin_mag[-1] - bin_mag[0])))))
        p_MFD.append(p_i)
...
def tapered_GR(mfd_param, bin_mag):
    ...
    for mag in bin_mag:
        ...
        p_i = (((M0_t / M0_min)**Beta * np.exp(
            (M0_t - M0_min) / M0_c)) - ((M0_t / M0_max)**Beta * np.exp(
                (M0_t - M0_max) / M0_c)))
        p_MFD.append(p_i)
...
def YC(mfd_param, bin_mag, Mmax):
    ...
    for mag in bin_mag:
        if mag < (Mmax - 0.5):
            pi = 0.1 * ((beta * np.exp(-beta * (mag - bin_mag[0]))) /
                        (1. - np.exp(-beta * (Mmax - bin_mag[0]))))
        elif mag < (Mmax + 0.1):
            pi = 0.1 * ((beta * (np.exp(-beta * (Mmax - 1.5 - bin_mag[0])))) /
                        (1 - np.exp(-beta * (Mmax - bin_mag[0]))))
        p_MFD.append(pi)
    return p_MFD
...
def YC_modified(mfd_param, bin_mag, Mmax):
    ...
    b_value = mfd_param['b_value']
    Mf = mfd_param[
        'Mf']  #Mmax with the best a priori fit, value for the Marmara sea
    size_of_bump = mfd_param[
        'size_of_bump']  #in order to modify the respective size of the two parts of the
    beta = b_value * np.log(10)
    '''
    if Mmax <= 7.8 :
    ...
    for mag in bin_mag:
        if mag < (Mmax - 0.8):
            pi = 0.1 * (
                ((beta * np.exp(-beta * (mag - bin_mag[0]))) /
                 (1. - np.exp(-beta *
                              (Mmax - bin_mag[0])))) * np.exp(1. / b_value *
                                                              (Mmax - Mf)))
        elif mag < (Mmax + 0.09):
            pi = 0.1 * ((
                (beta * (np.exp(-beta * (Mmax - 1.5 - bin_mag[0])))) /
                (1 - np.exp(-beta *
                            (Mmax - bin_mag[0])))) * scipy.stats.norm(0, 2).pdf(
                                (mag - (Mmax - 0.4)) * 5.) /
                        scipy.stats.norm(0, 2).pdf(0.)) * size_of_bump
        elif mag > (Mmax + 0.09):
            pi = 0.
        p_MFD.append(pi)
    return p_MFD
...
def user_defined(
    ...
    size_of_bump = mfd_param[
        'size_of_bump']  #in order to modify the respective size of the two parts of the distribution

    beta = b_value * np.log(10)
    #Mf= 6.4 #Mmax with the best a priori fit
    #size_of_bump = 1.

    self.calculation_log_file.write('\nUsing YC_modified with Mf=' + str(Mf) +
                                    ' and size_of_bump=' + str(size_of_bump) +
                                    '\n')
    ...
    for mag in bin_mag:
        if mag < (Mmax - 0.5):
            ...
            pi = 0.1 * ((
                (beta * (np.exp(-beta * (Mmax - 1.5 - bin_mag[0])))) /
                (1 - np.exp(-beta *
                            (Mmax - bin_mag[0])))) * scipy.stats.norm(0, 1).pdf(
                                (mag - (Mmax - 0.25)) * 5.) /
                        scipy.stats.norm(0, 1).pdf(0.)) * size_of_bump
        elif mag > (Mmax + 0.09):
            pi = 0.
        p_MFD.append(pi)
...
def UCERF_DV(bin_mag):
    mfd_ucerf_old = [
        0.0802928535, 0.006255187, 0.0811119499, 0.0158967773, 0.0852733969,
    ...
    mfd_ucerf_guessed = [
        0.1138501463, 0.0088694563, 0.1150115728, 0.0225406165, 0.1209122392,
        0.1606559551, 0.1207234632, 0.1150958638, 0.0244154936, 0.015844313,
        0.0130709194, 0.0032216881, 0.0041427486, 0.0104758837, 0.0226150741,
        0.0223162261, 0.1019376482, 0.0040203001, 0.0002803921
    ]

    mfd_ucerf = [
        0., 0.1225479995, 0.0095470667, 0.1237981981, 0.0181124989,
    ...
    mfd_ucerf = [
        0., 0.1170913873, 0.1180649338, 0.0434295744, 0.0431719421,
        0.0279491815, 0.1776244068, 0.1772698507, 0.0686700644, 0.0460331714,
        0.0073358841, 0.008953384, 0.0109225241, 0.0065276199, 0.0022473075,
        0.0059243291, 0.0092550551, 0.0662428769, 0.0627078821, 0.0005786247
    ]
