...
def get_rate_model(rup_rates, fault_prop, bin_mag):
    ...
    rate_bg_in_model = np.zeros(
        len(bin_mag))  #rate of EQ in the BG for each magnitude
    for index_mag in range(len(bin_mag)):
        rate_bg_in_model[index_mag] += (
            (1 - fault_prop(bin_mag[index_mag])) * rate_f_in_model[index_mag]
        ) / fault_prop(
            bin_mag[index_mag]
        )  #earthquake incremental rate of the BG in the model at the time of the calculation
