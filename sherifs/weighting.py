...
class Weight_LT():
    ...
    def initialize(self):
                    ...
                    for bg_hyp_i in self.bg_names:
                        for sc_name in self.sc_names:
                            for ScL_i in self.selected_ScL:
                                ...
                                branch_i = [
                                    model_i, ScL_name_i, use_all_i, dim_i,
                                    mfd_i, bvalue, bg_hyp_i, sc_name
                                ]
        ...
        Ligne = '\t\t<logicTreeBranchingLevel branchingLevelID="bl' + str(
            id_number) + '">\n'
        XMLfile.write(Ligne)
        ...
        for branch in branches:
            ...
            for sample in range(1, nb_random_sampling + 1):
                ...
                Ligne = ('\t\t\t\t<logicTreeBranch branchID="' + str(Model) +
                         '-' + str(BG_hyp) + '-' + str(selected_ScL) + '-' +
                         str(dim_used) + '-' + str_all_data + '-' +
                         str(scenario_set) + '-' + str(bvalue) + '-' +
                         str(mfd_hyp) + '-s_' + str(sample) + '">\n')
                XMLfile.write(Ligne)
                Ligne = ('\t\t\t\t\t<uncertaintyModel>' +
                         (str(Model) + '/' + str(BG_hyp) + '/' +
                          str(selected_ScL) + '_' + str(dim_used) + '_' +
                          str_all_data + '/' + str(scenario_set) + '/' +
                ...
                index = np.where(
                    np.array(self.selected_ScL) == branch[1] + ' ' + branch[3] +
                    ' ' + branch[2])[0][0]
                ...
                index_bvalue = np.where(
                    np.array(self.b_values_hyps[index_mfd]) == bvalue)[0][0]
                weight_bv = float(self.weight_b_value[index_mfd][index_bvalue])

                weight_branch = weight_model * weight_bg * weight_sc * weight_sl * weight_mfd * weight_bv / float(
                    nb_random_sampling)
                #print weight_branch
                Ligne = '\t\t\t\t\t<uncertaintyWeight>' + str(
                    round(float(weight_branch), 6)) + '</uncertaintyWeight>\n'
                check_weights.append(round(float(weight_branch), 6))

                XMLfile.write(Ligne)
                print(
                    str(Model) + '/' + str(BG_hyp) + '/' + str(selected_ScL) +
                    '_' + str(dim_used) + '_' + str_all_data + '/' +
                    str(scenario_set) + '/' + str(bvalue) + '/' + str(mfd_hyp) +
                    '   ' + str(round(float(weight_branch), 7)))
    ...
    def window_weight(self):
        ...
        for i in range(len(array_hypo)):
            ...
            self.weight["w_model_{0}".format(i)].insert(
                INSERT, str(round(1. / float(len(array_hypo)), 3)))
            row_i += 1
        ...
        for i in range(len(array_hypo)):
            ...
            self.weight["w_bg_{0}".format(i)].insert(
                INSERT, str(round(1. / float(len(array_hypo)), 3)))
            row_i += 1
        ...
        for i in range(len(array_hypo)):
            ...
            self.weight["w_sl_{0}".format(i)].insert(
                INSERT, str(round(1. / float(len(array_hypo)), 3)))
            row_i += 1
        ...
        for i in range(len(array_hypo)):
            ...
            self.weight["w_sc_{0}".format(i)].insert(
                INSERT, str(round(1. / float(len(array_hypo)), 3)))
            row_i += 1
        ...
        for i in range(len(array_hypo)):
            ...
            self.weight["w_mfd_{0}".format(i)].insert(
                INSERT, str(round(1. / float(len(array_hypo)), 3)))
            row_i += 1
        ...
        for i in range(len(self.mfd_hyps)):
            for j in range(len(array_hypo[i])):
                ...
                self.weight["w_bv_{0}".format(ii)].insert(
                    INSERT, str(round(1. / float(len(array_hypo[i])), 3)))
                row_i += 1
                ii += 1
            index_mfd += 1
    ...
    def OK(self):
        ...
        for i in range(len(self.mfd_hyps)):
            weight_b_value_i = []
            for j in range(len(array_hypo[i])):
                weight_b_value_i.append(
                    self.weight["w_bv_{0}".format(ii)].get())
                ii += 1
