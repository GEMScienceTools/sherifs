...
class bg():
    ...
    def geom(model_name, file_geom):
        ...
        if not ".geojson" in file_geom:
            ...
            geom_bg = np.genfromtxt(file_geom,
                                    dtype=[('U100'), ('f8'), ('f8')],
                                    skip_header=1)

            column_model = list(
                map(lambda i: geom_bg[i][0], range(len(geom_bg))))
            ...
            for bg_i in range(len(bgs)):
                if bgs[bg_i]['properties']['model'] == model_name:
                    Lon_bg = [
                        i[0] for i in bgs[bg_i]['geometry']["coordinates"][0][0]
                    ]
                    Lat_bg = [
                        i[1] for i in bgs[bg_i]['geometry']["coordinates"][0][0]
                    ]
