...
def mag_bin_distr(f_for_sherifs):
    ...
    binning_in_mag = np.array(
        [round(i, 1) for i in list(np.linspace(5, 10, 51))])
    nb_rup_per_bin = []
...
def build_scenarios(f_for_sherifs, id_sections_fault, sections_areas_tot,
                    sections_lengths_tot, binning_in_mag, nb_rup_per_bin,
                    section_jump):
    t0 = time.time()
    ...
    for si in range(nb_sections):
        ...
        if len(sec_fi) > 1:
            ...
            for i in range(len(sec_fi) - id_si_in_fi):
                ...
                max_length = min(
                    [f_for_sherifs[i]["max_possible_length"] for sk in new_rup])

                rup_area = sum([sections_areas_tot[sk] for sk in new_rup])
                max_mmax = min(
                    [f_for_sherifs[sk]["max_possible_Mmax"] for sk in new_rup])
                rake = sum([
                    sections_areas_tot[sk] * f_for_sherifs[sk]["rake"]
                    for sk in new_rup
                ]) / rup_area
                mag = wc1994_median_mag(rup_area, rake)
                index_mag = np.where(binning_in_mag == round(mag, 1))[0][0]
                        ...
                        if not new_rup in rup:
                            ...
                            if nb_fault_invo <= max_fault_invo:
                                ...
                                for sk in new_rup:
                                    ...
                                    if rup_length > f_for_sherifs[sk][
                                            "max_length"]:
                                        f_for_sherifs[sk][
                                            "max_length"] = rup_length
                                ...
                                if nb_fault_invo == 1:
                                    ...
                                    len_2 = len(id_sections_fault[f_for_sherifs[
                                        new_rup[0]]["oiler_id"]])
                                    if len_2 == len(new_rup):
                                        full_fault = True
                                full_fault_rup.append(full_fault)
    ...
    if target_section == True:
        print("look at section :", look_at,
              "\nother sections in the same fault",
              id_sections_fault[f_for_sherifs[look_at]["oiler_id"]],
              "\njumps with", section_jump[look_at], "\nruptures",
              f_for_sherifs[look_at]["rup_id"])
        print("ruptures :")
            ...
            for sj in jumps:
                ...
                for id_rup_i in sii_rup:
                    ...
                    if rups_length[id_rup_i] > max_length / 2.:
                        sjj_rup = [
                            j for j in sj_rup if full_fault_rup[j] == True
                        ]
                    else:
                        sjj_rup = [
                            j for j in sj_rup
                            if rups_length[j] < max_length / 2.
                        ]
                    ...
                    for id_rup_j in sjj_rup:
                        ...
                        nb_jumps = len(rup_f_jump[id_rup_j] +
                                       rup_f_jump[id_rup_i]) + 1
                        ...
                        if build == True:
                            jump_i = sorted([
                                f_for_sherifs[si]["oiler_id"],
                                f_for_sherifs[sj]["oiler_id"]
                            ])
                            nb_zig_zag = rup_f_jump[id_rup_j].count(
                                jump_i) + rup_f_jump[id_rup_i].count(jump_i)
                        ...
                        if build == True:
                            ...
                            new_rup = sorted(list(set(new_rup)))
                            rup_length = rups_length[id_rup_i] + rups_length[
                                id_rup_j]
                            max_length = min([
                                f_for_sherifs[sk]["max_possible_length"]
                                for sk in new_rup
                            ])
                            ...
                            max_mmax = min([
                                f_for_sherifs[sk]["max_possible_Mmax"]
                                for sk in new_rup
                            ])
                            rake = (rups_rake[id_rup_i] * rups_area[id_rup_i] +
                                    rups_rake[id_rup_j] *
                                    rups_area[id_rup_j]) / rup_area
                            mag = wc1994_median_mag(rup_area, rake)
                            index_mag = np.where(
                                binning_in_mag == round(mag, 1))[0][0]
                        ...
                        if build == True:
                            ...
                            if test_using_length == True:
                                ...
                                min_max_length_i = min([
                                    f_for_sherifs[i]["max_length"]
                                    for i in new_rup
                                ])
                                mean_max_length_i = np.mean([
                                    f_for_sherifs[i]["max_length"]
                                    for i in new_rup
                                ])
                                ...
                                if hogger == True:
                                    ...
                                    if rup_length < min_max_length_i / 2.:
                                        if len(rup[id_rup_j]) < 2 or len(
                                                rup[id_rup_i]) < 2:
                                            build = False
                                        else:
                                            nb_i_to_remove = 2
                                    elif rup_length < min_max_length_i * 3. / 4.:
                                        if len(rup[id_rup_j]) < 3 or len(
                                                rup[id_rup_i]) < 3:
                                            build = False
                                        else:
                                            nb_i_to_remove = 3
                                    else:
                                        if len(rup[id_rup_j]) < 4 or len(
                                                rup[id_rup_i]) < 4:
                                            build = False
                                        else:
                                            nb_i_to_remove = 4
                        ...
                        if build == True:
                            ...
                            rup_f_jump.append(rup_f_jump[id_rup_j] +
                                              rup_f_jump[id_rup_i] + [jump_i])
    ...
    print(
        "Max number of ruptures for a section :",
        max([
            len(f_for_sherifs[i]["rup_id"]) for i in range(len(f_for_sherifs))
        ]))
    print("Number of ruptures created for nothing because already there : ",
          already_created)
    print("Number of ruptures tried for nothing because already paired : ",
          already_paired)
    print("Number of ruptures not used because too many faults : ",
          too_many_faults)
    print("Number of ruptures not used because too many jumps : ",
          too_many_jumps)
    print("Number of ruptures not used because too many zigzag : ",
          too_many_zig_zag)
    print("Number of ruptures not used because not diverse enough : ",
          not_diverse_enough)
    print("Number of ruptures not used because too long : ", too_long)
    print("Number of ruptures not used because magnitude was too large : ",
          too_big)
    ...
    print("\n\nIt took ", round(duration, 3),
          "seconds to find all the ruptures.")
        ...
        if len(f_for_sherifs[s]["rup_id"]) > 3.:
            ...
            for rup_i in rup:
                ...
                if add == True:
                    geom = []
                    for si in rup_i:
                        for lon_i, lat_i in zip(f_for_sherifs[si]["lons"],
                                                f_for_sherifs[si]["lats"]):
