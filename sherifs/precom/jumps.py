...
def find_jumps(f_for_sherifs, assso_fault, id_sections_fault, jump_dist, path,
               force_jump_on_fault):
        ...
        for fj in assso_fault[fi]:
            if fi != fj:
                for sj in id_sections_fault[fj]:
                    ...
                    if jump == True:
                        azi = calculate_initial_compass_bearing(
                            [lats_i[0], lons_i[0]], [lats_i[-1], lons_i[-1]])
                        azj = calculate_initial_compass_bearing(
                            [lats_j[0], lons_j[0]], [lats_j[-1], lons_j[-1]])
                    ...
                    if (jump == False and f_for_sherifs[si]["oiler_name"]
                            == f_for_sherifs[sj]["oiler_name"] and
                            f_for_sherifs[si]["oiler_name"] != None and
                            f_for_sherifs[si]["oiler_name"]
                            in force_jump_on_fault):
                        shortest_pt_between_faults = 100000
                        for sk in id_sections_fault[fj]:
                            if (f_for_sherifs[si]["oiler_name"] ==
                                    f_for_sherifs[sk]["oiler_name"]):
                                ...
                                for sl in id_sections_fault[fi]:
                                    if (f_for_sherifs[si]["oiler_name"] ==
                                        ...
                                        for lon_l, lat_l in zip(lons_l, lats_l):
                                            for lon_k, lat_k in zip(
                                                    lons_k, lats_k):
                                                dist = distance(
                                                    lon_l, lat_l, lon_k, lat_k)
...
#                             print()
                    ...
                    if jump == True:
                        si_jump.append(sj)
                        file_jumps_1.write(
                            str(save[0]) + ',' + str(save[1]) + '\n')
                        file_jumps_2.write(
                            str(save[2]) + ',' + str(save[3]) + '\n')
                        file_jumps_line.write(
                            str(si) + ";LINESTRING(" + str(save[0]) + ' ' +
                            str(save[1]) + ',' + str(save[2]) + ' ' +
                            str(save[3]) + ')\n')
