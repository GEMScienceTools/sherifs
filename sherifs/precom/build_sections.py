...
def cut_faults(faults, f_lengths, f_areas, path, rupture_mesh_spacing):
    ...
    for fi in range(nb_faults):
        ...
        if len(lons_tot) < f_lengths[fi] / 3.:
            ...
            for i in range(len(lons_tot) - 1):
                di = distance(lons_tot[i], lats_tot[i], lons_tot[i + 1],
                              lats_tot[i + 1])
                nb_extra_pts = int(di / 3.)
                if i == len(lons_tot) - 2:
                    if nb_extra_pts > 1:
                        inter_lon_i += list(
                            np.linspace(lons_tot[i], lons_tot[i + 1],
                                        nb_extra_pts))
                        inter_lat_i += list(
                            np.linspace(lats_tot[i], lats_tot[i + 1],
                                        nb_extra_pts))
                    else:
        ...
        if f_lengths[fi] < 35.:  #TODO clean up hard variables
            ...
            file_section_tips.write(
                str(lons_tot[0]) + "," + str(lats_tot[0]) + "\n")
            file_section_tips.write(
                str(lons_tot[-1]) + "," + str(lats_tot[-1]) + "\n")
            ...
            for i in range(len(lons_tot) - 1):
                dists.append(
                    distance(lons_tot[i], lats_tot[i], lons_tot[i + 1],
                             lats_tot[i + 1]))
            if f_lengths[fi] < 125.:
                if max(dists) > 25.:
            ...
            if nb_sections != 1:
                ...
                for cut in range(nb_sections - 1):
                    ...
                    while dist < av_section_len:
                        try:
                            dist += distance(lons_tot[index_cut],
                                             lats_tot[index_cut],
                                             lons_tot[index_cut + 1],
                                             lats_tot[index_cut + 1])
                            index_cut += 1
                            do_last = True

                            # check for major azimut change
                            if dist > av_section_len * 0.3 and dist > 2. * rupture_mesh_spacing:
                                az1 = calculate_initial_compass_bearing([
                                    lons_tot[index_cut - 1],
                                    lats_tot[index_cut - 1]
                                ], [lons_tot[index_cut], lats_tot[index_cut]])
                                az2 = calculate_initial_compass_bearing(
                                    [lons_tot[index_cut], lats_tot[index_cut]],
                                    [
                                        lons_tot[index_cut + 1],
                                        lats_tot[index_cut + 1]
                    ...
                    for i in range(len(lons) - 1):
                        length += distance(lons[i], lats[i], lons[i + 1],
                                           lats[i + 1])
                    ...
                    file_section_tips.write(
                        str(lons[0]) + "," + str(lats[0]) + "\n")
                    file_section_tips.write(
                        str(lons[-1]) + "," + str(lats[-1]) + "\n")
            ...
            #print("#####\n fault",fi)
            if round(sum(sections_lengths_tot[-nb_sections:])) != round(
                    f_lengths[fi]):
                print("#####\n fault", fi)
                print("Failing!")
                print(
                    round(sum(sections_lengths_tot[-nb_sections:])),
                    round(f_lengths[fi]), "missing",
                    round(-sum(sections_lengths_tot[-nb_sections:])) +
                    round(f_lengths[fi]))
    ...
    if round(sum(sections_lengths_tot)) != round(sum(f_lengths)):
        print("!!!!!\n ERROR! There are ",
              round(sum(f_lengths) - sum(sections_lengths_tot)),
              " km of faults missing.")
...
def to_sherifs(f_for_sherifs, faults, Model_name, apply_sr_reduction, f_mu):
    ...
    for si in range(nb_sections):
        ...
        if "v_rl" in faults[fi]['properties'].keys():
            ...
            err = (faults[fi]['properties']["e_rl"]**2 +
                   faults[fi]['properties']["e_ex"]**2)**0.5
        ...
        f_for_sherifs[si]["slip_rate_min"] = slip_rate_min * (
            1. - apply_sr_reduction)
        f_for_sherifs[si]["slip_rate_moy"] = slip_rate_moy * (
            1. - apply_sr_reduction)
        f_for_sherifs[si]["slip_rate_max"] = slip_rate_max * (
            1. - apply_sr_reduction)
