...
class OQ_job_Creator():
    ...
    def input_data(self):
        ...
        if os.path.exists(info_run_file):  #the file exist, read it
            ...
            self.Mmax_range = [
                float(lines_of_the_file[19].split('\t')[1]),
                float(lines_of_the_file[19].split('\t')[2])
            ]
...
#            self.write_info()
    ...
    def write_ini(self):
        ...
        if self.option_map == 'yes':
            ...
            jobfile.write(Ligne)
            Ligne = 'region_grid_spacing = ' + str(
                self.region_grid_spacing) + '\n'
        else:
            jobfile.write(Ligne)
            Ligne = 'sites = ' + str(self.Longitude)[:-1] + ' ' + str(
                self.Latitude) + '\n'
        jobfile.write(Ligne)
        Ligne = '\n'
        jobfile.write(Ligne)
        ...
        jobfile.write(Ligne)
        intensities_done = [
        ]  #check to see if we don't put twice the same intensity
        for i in range(len(self.intensity_out)):
            ...
            array_ints = np.logspace(np.log10(self.min_int_out[i]),
                                     np.log10(self.max_int_out[i]),
                                     self.nb_int_out[i])
            Ligne_2 = ''
