    ...
    if type_of_fault == 'sf':
        ...
        if do_resample == True:
            ...
            if plot_stuf_detail == True:
                min_dist_tmp = min_d
                for i_pt in range(len(ColLon) - 2):
                    dist_tmp = distance(resampled_ColLon[i_pt + 1],
                ...
                if min_dist_tmp < min_d / 2.:
                    print("min dist :", round(min_dist_tmp), "id: ",
        ...
        for x, y in zip(ColLon, ColLat):
            if use_local_azimuth == True:
                ...
                if [x, y] == [ColLon[0], ColLat[0]]:
                    ...
                    compass_bearing = calculate_initial_compass_bearing(
                        (ColLat[-2], ColLon[-2]), (ColLat[-1], ColLon[-1]))
                    strike = compass_bearing
                    azimuth = (strike + 90.0) % 360
                else:
                    compass_bearing = calculate_initial_compass_bearing(
...
def wrt_source(txt, section_id, faults_names, faults_data, geotype, resample,
               vertical_faults, trt, M_min, MFD, ScL_oq):
    '''
    txt : str containing the file info
    ...
    txt += '    <kiteFaultSource name="' + section_name + '" id="' + str(
        section_id) + '" tectonicRegion="' + trt + '">\n'
    ...
    txt += '        <incrementalMFD minMag=\"' + str(
        M_min) + '\" binWidth=\"0.10\">\n'
    txt += '        <occurRates> ' + ' '.join(list(map(
        str, MFD))) + '</occurRates>\n'
    txt += '        </incrementalMFD>\n'
    txt += '        <rake> ' + str(
        faults_data[section_id]['rake']) + '</rake>\n'
    txt += '        </kiteFaultSource>\n'
    return txt
