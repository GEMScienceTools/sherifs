...
class Source_Model_Creator:
    ...
    def __init__(self, path, pathlog, param, Model_name, rupture_set, sample,
        ...
        if param["main"]["background"]["option_bg"] == "zone":
            if param["main"]["background"]["use_host_model"] in [
                    "true", "True"
            ]:
                self.use_host_model = True
                self.host_model_file = param["main"]["background"][
                    "host_model_file"]
    ...
    def initialize(self):
        ...
        log_sr_file = open(
            self.pathlog + '/slip_rate_sample_' + str(self.sample) + '.txt',
            'w')
        log_mdf_file = open(
            self.pathlog + '/mdf_sample_' + str(self.sample) + '.txt', 'w')
        ...
        if cut_sm_file == False:
            XMLfile = open(
                self.path + '/Source_model_' + str(self.sample) + '.xml', 'w')
        else:
        ...
        if "include_all_faults" in self.param["main"]["background"].keys():
            if self.param["main"]["background"]["include_all_faults"] in [
                    "True", "true"
            ]:
        ...
        if use_multiF == True:
            ...
            #######################################
            '''
            simplify_faults = self.param["main"]["parameters"][
                "simplify_faults"]
            ...
            if "vertical_faults" in self.param["main"]["parameters"].keys():
                vertical_faults = self.param["main"]["parameters"][
                    "vertical_faults"]
            ...
            log_general_parameters_file = open(
                self.pathlog + '/general_parameters_sample_' +
                str(self.sample) + '.txt', 'w')
            ...
            log_general_parameters_file.write('b_value\t' +
                                              str(mfd_param["b_value"]) + '\n')
            ...
            if self.sr_correl == True:
                        ...
                        for j in range(len(faults_names)):
                            if M_linked_lvl[i][j] == 0:
                                if M_faults_correl[i][j] != 0:
                                    ...
                                    for jj in range(
                                            len(M_faults_correl[j])
                                    ):  #for all faults connecting to a fault connected to i
                ...
                for Fault_name in faults_names:
                    ...
                    sr_values = [
                        faults_data[index_fault]['slip_rate_min'],
                        faults_data[index_fault]['slip_rate_moy'],
                        faults_data[index_fault]['slip_rate_max']
                    ]

                    #selects randomly the slip rate
                    slip_rate = select_sr.select(sr_values, self.sample,
                                                 index_fault,
                                                 M_linked_lvl[index_fault],
                                                 list_quater_picked)

                    log_line = str(Fault_name) + '\t' + str(
                        slip_rate) + '\n'  #writting in the log file
                    log_sr_file.write(log_line)

                    faults_slip_rates.append(slip_rate *
                                             0.001)  # transform from mm to m

                    #                faults_shear_mod.append(float(self.shear_mod)*10**9 )
                    faults_shear_mod.append(
                        faults_data[index_fault]['shear_mod'])
                    index_fault += 1
                ...
                for Fault_name in faults_names:
                    ...
                    if self.sample == 1:
                        ...
                        slip_rate_inf = np.random.uniform(
                            slip_rate_min, slip_rate_moy)
                        slip_rate_sup = np.random.uniform(
                            slip_rate_moy, slip_rate_max)
                        slip_rate = np.random.choice(
                            [slip_rate_inf, slip_rate_sup])
            ...
            if re_use_mfd_pkl == False:
                while abs(ratio_test - 1) > self.fit_quality or math.isnan(
                        ratio_test) == True:
                    MFDs = EQ_on_faults.EQ_on_faults_from_sr(
                        self.Run_Name, M_min, mfd_param, faults_names,
                        faults_area, faults_length, faults_width,
                        faults_slip_rates, scenarios_names, faults_shear_mod,
                        self.path, self.pathlog, self.sample, self.selected_ScL,
                        self.dimention_used, self.use_all_ScL_data,
                        faults_mecanism, self.bg_ratio, self.size_of_increment,
                        self.mfd_hyp, count_reruns, faults_lon, faults_lat,
                        self.Mmax_range, self.calculation_log_file, self.branch,
                        self.param)
                ...
                with open(f_pkl_mdf, 'wb') as f:
                    MFDs_to_pkl = [
                        MFDs.EQ_rate_BG, MFDs.faults_names,
                        MFDs.OQ_entry_faults, MFDs.scenarios_names,
                        MFDs.OQ_entry_scenarios, MFDs.index_faults_in_scenario,
            ...
            try:
                ...
                print(
                    "ERROR, please make sure the model name matches in the bg file"
                )

            # find the faults in the bg

            f_in_bg = []
            for index_fault, fault_name in zip(range(len(faults_names)),
                                               faults_names):
                for lon, lat in zip(faults_data[index_fault]['lon'],
                                    faults_data[index_fault]['lat']):
            ...
            # write source file
            if cut_sm_file == False:
                source_xml = self.path + '/Source_model_' + str(
                    self.sample) + '.xml'
                ...
                txt = wmfs.start_multifault_source(txt, name, trt, sections_xml,
                                                   source_id)
            else:
                for f in s_files:
                    trt = faults_data[0]['domain']
                    ...
                    dict_txt[f] = wmfs.start_multifault_source(
                        dict_txt[f], name, trt, sections_xml, source_id)
                    source_id += 1
                # working file
            ...
            if len(self.rupture_set) != 0:
                for scenario in enumerate(scenarios_names):
                    ...
                    sc_in = False
                    index_scenario = np.where(
                        np.array(scenarios_names) == scenario[1])[0][0]
                    index_faults_in_sc = index_faults_in_scenario[
                        index_scenario][0]
                    ...
                    if sc_in == True:
                        ...
                        if sum(MFD) != 0:
                            faults_in_scenario = np.take(
                                faults_names, index_faults_in_sc)
                            ...
                            if cut_sm_file == False:
                                txt = wmfs.wrt_multifault_source(
                                    txt, MFD, self.Mmin, explo_time, rake,
                                    index_faults_in_sc)
                            else:
                                if not dict_n_rup[f] < max_rup_per_file:
                                    i = 0
                                    while dict_n_rup[f] > max_rup_per_file:
                                        ...
                                        f = list(dict_txt.keys())[i]
                                txt = dict_txt[f]
                                dict_txt[f] = wmfs.wrt_multifault_source(
                                    txt, MFD, self.Mmin, explo_time, rake,
                                    index_faults_in_sc)
                                dict_n_rup[f] += 1
            ...
            #initialisation of the general parameters (M_min, shear modulus and b value)
            log_general_parameters_file = open(
                self.pathlog + '/Log/general_parameters_sample_' +
                str(self.sample) + '.txt', 'w')
            ...
            if re_use_mfd_pkl == False:
                while abs(ratio_test - 1) > self.fit_quality or math.isnan(
                        ratio_test) == True:
                    MFDs = EQ_on_faults.EQ_on_faults_from_sr(
                        M_min, mfd_param, faults_names, faults_area,
                        faults_length, faults_width, faults_slip_rates,
                        scenarios_names, faults_shear_mod, self.path,
                        self.sample, self.selected_ScL, self.dimention_used,
                        self.use_all_ScL_data, faults_mecanism, self.bg_ratio,
                        self.size_of_increment, self.mfd_hyp, count_reruns,
                        faults_lon, faults_lat, self.Mmax_range,
                        self.calculation_log_file, self.branch, self.param)
                    ratio_test = MFDs.ratio_test
                    if abs(ratio_test - 1) > self.fit_quality:
                        print('bad sampling => re-run')
                        count_reruns += 1
            ...
            for index_fault, fault_name in zip(range(len(faults_names)),
                ...
                if index_fault in f_in_bg:
                    ...
                    if use_simple_faults == True:
                        line, self.Domain_in_the_model, ID_number = fault_source.write_simple_fault(
                            index_fault, fault_name, OQ_entry_faults,
                            faults_names, faults_data, self.Model_name,
                            self.Domain_in_the_model, ScL_oq, log_mdf_file,
                            M_min, ID_number)
                    else:
                        line, self.Domain_in_the_model, ID_number = fault_source.write_non_parametric_one_fault(
                            index_fault, fault_name, OQ_entry_faults,
                            faults_names, faults_data, self.Model_name,
                            self.Domain_in_the_model, ScL_oq, log_mdf_file,
                            M_min, ID_number, explo_time)
                ...
                for scenario in enumerate(scenarios_names):
                    ...
                    if sc_in == True:
                        ...
                        if use_non_param == False:
                            line, ID_number = fault_source.write_characteristic_scenario(
                                scenarios_names, OQ_entry_scenarios,
                                index_faults_in_scenario, scenario,
                                faults_names, self.Model_name, faults_data,
                                log_mdf_file, M_min, ID_number)
                        else:

                            line, ID_number = fault_source.write_non_parametric_source(
                                scenario, scenarios_names, OQ_entry_scenarios,
                                index_faults_in_scenario, faults_names,
                                faults_data, self.Model_name,
                                self.Domain_in_the_model, ScL_oq, log_mdf_file,
                                explo_time, M_min, ID_number)
                        if cut_sm_file == False:
                            XMLfile.write(line)
                        else:
                            #find index to write in
        ...
        if sum(MFD) != 0. and self.param["main"]["background"][
                "option_bg"] == "zone":
            ...
            upperSeismoDepth, lowerSeismoDepth, ruptAspectRatio, nodalPlanes, hypoDepths = bg.prop(
                self.Model_name, self.file_prop_bg)
            line += '\t\t<sourceGroup\nname="group 2"\nrup_interdep="indep"\nsrc_interdep="indep"\ntectonicRegion="' + str(
                self.Domain_in_the_model[0]) + '"\n>\n'
            line += '\t\t<areaSource id="' + str(
                source_id + 1) + '" name="Background" tectonicRegion="' + str(
                    self.Domain_in_the_model[0]) + '">\n'
            line += '\t\t\t<areaGeometry>\n'
            line += '\t\t\t\t<gml:Polygon>\n'
            line += '\t\t\t\t\t<gml:exterior>\n'
            ...
            line += '\t\t\t\t\t</gml:exterior>\n'
            line += '\t\t\t\t</gml:Polygon>\n'
            line += '\t\t\t\t<upperSeismoDepth>' + str(
                upperSeismoDepth) + '</upperSeismoDepth>\n'
            line += '\t\t\t\t<lowerSeismoDepth>' + str(
                lowerSeismoDepth) + '</lowerSeismoDepth>\n'
            line += '\t\t\t</areaGeometry>\n'
            line += '\t\t\t<magScaleRel>' + ScL_oq + '</magScaleRel>\n'
            line += '\t\t\t<ruptAspectRatio>' + str(
                ruptAspectRatio) + '</ruptAspectRatio>\n'
            log_mdf_file.write('Background' + '\t' + str(M_min) + '\t' +
                               ' '.join(list(map(str, MFD))) + '\n')
            line += '\t\t\t<incrementalMFD binWidth=\"0.10\" minMag="' + str(
                M_min) + '">\n'
            line += '\t\t\t<occurRates> ' + ' '.join(list(map(
                str, MFD))) + '</occurRates>\n'
            line += '\t\t\t</incrementalMFD>\n'
            line += '\t\t\t<nodalPlaneDist>\n'
            for i in range(len(nodalPlanes)):
                line += ('\t\t\t\t<nodalPlane probability="' +
                         str(nodalPlanes[i][0]) + '" strike="' +
                         str(nodalPlanes[i][1]) + '" dip="' +
                         str(nodalPlanes[i][2]) + '" rake="' +
                         str(nodalPlanes[i][3]) + '" />\n')
            line += '\t\t\t</nodalPlaneDist>\n'
            line += '\t\t\t<hypoDepthDist>\n'
            for i in range(len(hypoDepths)):
                line += '\t\t\t\t<hypoDepth probability="' + str(
                    hypoDepths[i][0]) + '" depth="' + str(
                        hypoDepths[i][1]) + '" />\n'
            line += '\t\t\t</hypoDepthDist>\n'
            line += '\t\t</areaSource>\n'
            line += '\t</sourceGroup>\n'
        ...
        elif sum(MFD) != 0. and self.param["main"]["background"][
                "option_bg"] == "smooth":
            Mmin_checked = False
            ...
            if os.path.isdir(self.fbgpath):
                list_bg_xml = [
                    self.fbgpath + "/" + i for i in list_bg_xml if '.xml' in i
                ]
                ...
                for pointSource in nrml[0][0]:
                    ...
                    for child in pointSource.getchildren():
                        if "pointGeometry" in str(child):
                            ...
                            s_tmp = s_tmp.replace('\n', '')
                            s_tmp = [
                                float(i) for i in s_tmp.split(' ') if i != ''
                            ]
                            if bbPath_BG.contains_point(
                                (s_tmp[0], s_tmp[1])) == 1:
                                pt_in_BG = True
                    ...
                    if pt_in_BG == True:
                        ...
                        if Mmin_checked == False:
                            if float(minMag) < M_min:
                                print("!!!!")
                                print(
                                    "WARNING : BG has a smaller Mmin than the SHERIFS input"
                                )
                                print("!!!!")
                                Mmin_checked = True
                        ...
                        if include_all_faults == True:
                            ...
                            i = 0
                            while pt_very_far == False and i < len(
                                    outside_faults):
                                i_fault = outside_faults[i]
                                mean_lon = np.mean(
                                    faults_data[index_fault]['lon'])
                                mean_lat = np.mean(
                                    faults_data[index_fault]['lat'])
                                if abs(float(s_tmp[0]) -
                                       mean_lon) < lon_lat_distance_criteria:
                                    pt_very_far = False
                                if abs(float(s_tmp[1]) -
                                       mean_lat) < lon_lat_distance_criteria:
                                    pt_very_far = False
                                i += 1
                            ...
                            if pt_very_far == False:
                                ...
                                for i_fault in outside_faults:
                                    ...
                                    mean_lat = np.mean(
                                        faults_data[index_fault]['lat'])
                                    dist_i = distance(mean_lon, mean_lat,
                                                      float(s_tmp[0]),
                                                      float(s_tmp[1]))
                                    if dist_i < dist_critera:
                                        for lon_f, lat_f in zip(
                                                faults_data[index_fault]['lon'],
                                                faults_data[index_fault]
                                            ['lat']):
                                            dist_i = distance(
                                                mean_lon, mean_lat,
                                                float(s_tmp[0]),
                    ...
                    for str_loc in pts_out_list.keys():
                        if pts_out_list[str_loc]["closest_fault"] == i_fault:
                            if pts_out_list[str_loc]["closest_dist"] < buffer_1:
                                nb_buf1 += 1
                            elif pts_out_list[str_loc][
                                    "closest_dist"] < buffer_2:
                                nb_buf2 += 1
                    nb_pt_in_buff.update(
                        {i_fault: {
                            "nb_buf1": nb_buf1,
                            "nb_buf2": nb_buf2
                        }})
            ...
            for fbg in list_bg_xml:
                ...
                for pointSource in nrml[0][0]:
                    ...
                    if str_loc in pts_list.keys(
                        ...
                        element = nrml[0][0][i_point].makeelement(
                            'incrementalMFD', attrib)
                        nrml[0][0][i_point].append(element)
                        element = nrml[0][0][i_point][-1].makeelement(
                            'occurRates', {})
                    ...
                    if include_all_faults == True:
                        if str_loc in pts_out_list.keys():
                            ...
                            for mag in mags:
                                ...
                                reduction = 0.
                                for f_in_rup, mfd_i in zip(
                                        id_ruptures, mfd_mlt_clst_f):
                                    reduction += mfd_i[i_mag] / float(
                                        len(f_in_rup) *
                                        nb_pt_in_buff[i_fault][buffer_to_use])
                                if reduction > r:
                                    r = 0.
                                else:
                                    r -= reduction
                            ...
                            pts_out_list[str_loc].update(
                                {"scaled_mfd": pt_scl_mfd})

                            nrml[0][0][i_point][-1][0].text = str_tmp
                            nrml[0][0][i_point].remove(
                                nrml[0][0][i_point][i_trGR])

                    i_point += 1
                i_bg += 1
                fbg_out = self.path + '/bg_' + str(
                    self.sample) + '_' + str(i_bg) + '.xml'
                tree.write(fbg_out)
                list_src_files.append(fbg_out)
        '''#############################
        ### defining the other sources based on the host model
        ##############################'''
        if self.param["main"]["background"][
                "use_host_model"] == True and cut_sm_file == False:
            host_model.build(XMLfile, self.host_model_file, Lon_bg, Lat_bg)
        if self.param["main"]["background"][
                "use_host_model"] == True and cut_sm_file == True:
        ...
        for si in range(len(faults_data)):
            sections = []
            geom = []
            for lon_i, lat_i in zip(faults_data[si]["lon"],
                                    faults_data[si]["lat"]):
        ...
        if plt_model_mfd == True:
            ...
            lim = [[x[0] - 0.05, x[-1] + 0.05], [min(y) / 2., max(y) * 2.]]
        ...
        if plt_model_mfd == True:
            x = MFDs.bin_mag
            ft, bgmfd = rates.get_rate_faults_n_bg(MFDs.rup_rates,
                                                   MFDs.fault_prop, x)
            ys = [
                rates.get_rate_model(MFDs.rup_rates, MFDs.fault_prop, x), ft,
                bgmfd
            ]
        ...
        if part_mfd == True:
            loc_cat = local_cat.read_geojson(
                self.param["figures"]["parts_gjson"])
            ...
            for zone in loc_cat.keys():
                ...
                txt_no_bg, rate_faults, rate_bg, smooth = local_cat.get_model_rate(
                    poly, OQ_entry_faults, OQ_entry_scenarios, pts_list,
                    MFDs.bin_mag, self.param, faults_data, faults_names,
                    index_faults_in_scenario, local_zone_mfd)
                ...
                plt_mfd.local(x, [rate_model, rate_faults, rate_bg, smooth],
