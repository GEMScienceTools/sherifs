...
def write_simple_fault(index_fault, fault_name, OQ_entry_faults, faults_names,
    ...
    if fault_name in faults_names:
        ...
        if type_of_fault == 'sf':
            ...
            line = '\t\t<simpleFaultSource id="' + str(
                ID_number) + '" name="' + str(
                    fault_name) + '" tectonicRegion="' + str(
                        faults_data[index_fault]['domain']) + '">\n'
            test_ok += 1

            # orienting the arrays in order to respect OQ right hand rule
            compass_bearing = calculate_initial_compass_bearing(
                (ColLat[0], ColLon[0]), (ColLat[-1], ColLon[-1]))
            ...
            if use_kite == True:
                ...
                dip_tan = math.tan(math.radians(
                    faults_data[index_fault]['dip']))
                ...
                line += '\t\t\t\t<dip>' + str(
                    faults_data[index_fault]['dip']) + '</dip>\n'
                line += '\t\t\t\t<upperSeismoDepth>' + str(
                    faults_data[index_fault]
                    ['upper_sismo_depth']) + '</upperSeismoDepth>\n'
                line += '\t\t\t\t<lowerSeismoDepth>' + str(
                    faults_data[index_fault]
                    ['lower_sismo_depth']) + '</lowerSeismoDepth>\n'
                line += '\t\t\t</simpleFaultGeometry>\n'

        if type_of_fault == 'cf':
            fault_name = Model_name + '_' + str(fault_name)
            line = '\t\t<complexFaultSource id="' + str(
                ID_number) + '" name="' + str(
                    fault_name) + '" tectonicRegion="' + str(
                        faults_data[index_fault]['domain']) + '">\n'
            test_ok += 1
            ...
            for depth_i in sorted(set(Depth)):
                ...
                for index in indexes_for_edge_i:
                    line += '\t\t\t\t\t\t' + str(ColLon[index]) + ' ' + str(
                        ColLat[index]) + ' ' + str(Depth[index]) + '\n'
        ...
        if test_ok == 0:
            print(
                '!!!!!!!!!! Problem with the fault Geometry, please check input file'
                '')
            sys.exit()
        ...
        ########################################################
        line += '\t\t\t<incrementalMFD minMag=\"' + str(
            M_min) + '\" binWidth=\"0.10\">\n'
        if sum(MFD) != 0:
            log_mdf_file.write(
                str(fault_name) + '\t' + str(M_min) + '\t' +
        ...
        line += '\t\t\t<rake>' + str(
            faults_data[index_fault]['rake']) + '</rake>\n'
        if type_of_fault == 'sf':
...
def write_characteristic_scenario(scenarios_names, OQ_entry_scenarios,
                                  index_faults_in_scenario, scenario,
                                  faults_names, Model_name, faults_data,
                                  log_mdf_file, M_min, ID_number):
    ...
    if sum(MFD) != 0:
        ...
        scenar_name = '_'.join(
            "{!s}={!r}".format(key, val) for (key, val) in scenario[1].items())
        Fault_Name = Model_name + '_scenario_' + str(scenar_name)
        line = '\t\t<characteristicFaultSource id="' + str(
            ID_number) + '" name="' + str(
                Fault_Name) + '" tectonicRegion="' + str(
                    faults_data[index_fault]['domain']) + '">\n'
...
def write_non_parametric_one_fault(index_fault, fault_name, OQ_entry_faults,
                                   faults_names, faults_data, Model_name,
                                   Domain_in_the_model, ScL_oq, log_mdf_file,
    ...
    if sum(MFD) != 0:
        ...
        line = '\t\t<nonParametricSeismicSource id="' + str(
            ID_number) + '" name="' + str(
                Fault_Name) + '" tectonicRegion="' + str(
                    faults_data[index_fault]['domain']) + '">\n'
        ...
        bin_mag = np.linspace(M_min,
                              M_min + 0.1 * len(MFD) + 0.1,
                              num=2 + len(MFD))
        for mag, i_mag in zip(bin_mag, range(len(bin_mag))):
            if i_mag <= len(MFD) - 1:
                ...
                if l != 0.:
                    ...
                    line += '\t\t\t<multiPlanesRupture probs_occur="' + str(
                        p_not_occur) + ' ' + str(xxx1) + '">\n'
                    ...
                    if str_geom == '':
                        ...
                        if type_of_fault == 'sf':
                            ...
                            # Similar to :meth:`from_fault_data`, we just don't resample edges
                            dip_tan = math.tan(
                                math.radians(faults_data[index_fault]['dip']))
                            hdist_top = usd / dip_tan
                            hdist_bottom = lsd / dip_tan
                            ...
                            if str('N') in str(
                                    faults_data[index_fault]['oriented']):
                                if compass_bearing < 180.:
                                    ColLon = reversed(ColLon)
                                    ColLat = reversed(ColLat)
                            elif str('S') in str(
                            ...
                            if do_resample == True:
                                ...
                                resampled_ColLon, resampled_ColLat = [
                                    ColLon[0]
                                ], [ColLat[0]]
                                for i_pt in range(len(ColLon) - 2):
                                    ...
                                    dist_before = distance(
                                        resampled_ColLon[-1],
                                        resampled_ColLat[-1], ColLon[i_pt],
                                        ColLat[i_pt])
                                    dist_after = distance(
                                        resampled_ColLon[-1],
                                        resampled_ColLat[-1], ColLon[i_pt + 1],
                                        ColLat[i_pt + 1])

                                    az_before = calculate_initial_compass_bearing(
                                        (resampled_ColLon[-1],
                                         resampled_ColLat[-1]),
                                        (ColLon[i_pt], ColLat[i_pt]))
                                    az_after = calculate_initial_compass_bearing(
                                        (ColLon[i_pt], ColLat[i_pt]),
                                        (ColLon[i_pt + 1], ColLat[i_pt + 1]))

                                    if dist_before > min_d and (
                                            dist_after > max_d or
                                            abs(az_before - az_after) > az_d):
                                        add_point = True
                                        #special rule for the antepenultimate point
                                        if i_pt == len(ColLon) - 2:
                                            last_dist_after = distance(
                                                ColLon[i_pt], ColLat[i_pt],
                                                ColLon[-1], ColLat[-1])
                                ...
                                if plot_stuf_detail == True:
                                    ...
                                    for i_pt in range(len(ColLon) - 2):
                                        dist_tmp = distance(
                                            resampled_ColLon[i_pt + 1],
                                            resampled_ColLat[i_pt + 1],
                                            resampled_ColLon[i_pt],
                                            resampled_ColLat[i_pt])
                                        if dist_tmp < min_dist_tmp:
                                            min_dist_tmp = dist_tmp
                                    if min_dist_tmp < min_d / 2.:
                                        print("min dist :", round(min_dist_tmp),
                                              "id: ",
                                              faults_data[index_fault]['name'])
                    ...
                    hypo_depth = (
                        faults_data[index_fault]['upper_sismo_depth'] +
                        faults_data[index_fault]['lower_sismo_depth']) / 2.

                    str_geom += '\t\t\t<hypocenter depth="' + str(
                        hypo_depth) + '" lat="' + str(
                            np.mean(faults_data[index_fault]['lat'])
                        ) + '" lon="' + str(
                            np.mean(faults_data[index_fault]['lon'])) + '"/>\n'
...
def write_non_parametric_source(scenario, scenarios_names, OQ_entry_scenarios,
                                index_faults_in_scenario, faults_names,
                                faults_data, Model_name, Domain_in_the_model,
                                ScL_oq, log_mdf_file, explo_time, M_min,
                                ID_number):
    '''
    Write a non parametric source with fault trace defined as set of kite fault surfaces
    for now the rupture occurs once of not. several occurences are not supported yet.
                    ...
                    if str_geom == '':
                        ...
                        for Fault_name in faults_in_scenario:
                            ...
                            if Depth and all(elem == 'sf' for elem in Depth):
                                ...
                                Depth = [float(i) for i in Depth]
                            if type_of_fault == 'sf':
                                usd = faults_data[index_fault][
                                    'upper_sismo_depth']
                                lsd = faults_data[index_fault][
                                    'lower_sismo_depth']
                                # Similar to :meth:`from_fault_data`, we just don't resample edges
                                dip_tan = math.tan(
                                    math.radians(
                                ...
                                if do_resample == True:
                                    ...
                                    for i_pt in range(len(ColLon) - 2):
                                        ...
                                        if add_point == True:
                                            #add the point
                                            resampled_ColLon.append(
                                                ColLon[i_pt])
                                            resampled_ColLat.append(
                                                ColLat[i_pt])
...
def write_non_parametric_source_multi_types(
        scenario, scenarios_names, OQ_entry_scenarios, index_faults_in_scenario,
        faults_names, faults_data, Model_name, Domain_in_the_model, ScL_oq,
        log_mdf_file, explo_time, M_min, ID_number):
                        ...
                        for Fault_name in faults_in_scenario:
                            ...
                            if type_of_fault == 'sf':
                                ...
                                str_geom += '\t\t\t\t<dip>' + str(
                                    faults_data[index_fault]
                                    ['dip']) + '</dip>\n'
                                str_geom += '\t\t\t\t<upperSeismoDepth>' + str(
                                    faults_data[index_fault]
                                    ['upper_sismo_depth']
                                ) + '</upperSeismoDepth>\n'
                                str_geom += '\t\t\t\t<lowerSeismoDepth>' + str(
                                    faults_data[index_fault]
                                    ['lower_sismo_depth']
                                ) + '</lowerSeismoDepth>\n'
                                str_geom += '\t\t\t</simpleFaultGeometry>\n'
                            ...
                            if type_of_fault == 'cf':
                                ...
                                index_edge = 0
                                for depth_i in sorted(set(Depth)):
                                    indexes_for_edge_i = np.where(
                                        np.array(Depth) == depth_i)[0]
                                    if index_edge == 0:
