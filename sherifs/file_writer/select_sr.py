    ...
    if sample == 1:
        ...
        if sum(linked_level) == 0:  #the fault is alone
            ...
            if quarters_picked == []:  #none of the faults have bin picked yet
                ...
                quarter_to_pick = max(set(quarters_picked),
                                      key=quarters_picked.count)
                if quarter_to_pick == 1:
                    slip_rate = np.random.uniform(
                        sr_min,
                        sr_min + 1. / 2. * (sr_mean - sr_min) + 0.000001)
                elif quarter_to_pick == 2:
                    slip_rate = np.random.uniform(
                        sr_min + 1. / 2. * (sr_mean - sr_min),
                        sr_mean + 0.000001)
                elif quarter_to_pick == 3:
                    slip_rate = np.random.uniform(
                        sr_mean,
                        sr_mean + 1. / 2. * (sr_max - sr_mean) + 0.000001)
                elif quarter_to_pick == 4:
                    slip_rate = np.random.uniform(
                        sr_mean + 1. / 2. * (sr_max - sr_mean),
                        sr_max + 0.000001)
